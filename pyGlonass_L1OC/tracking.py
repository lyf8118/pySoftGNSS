...
--------------------------------
GLONASS L1OC code and carrier tracking.
...
class Channel:
    ...
    The results contain data- and pilot-channel prompt outputs, absolute L1OC
    ...
    def __init__(self, settings):
        ...
        self.lockFlag = False
        # Absolute input-sample index at the start of each L1OC code epoch.
        ...
        self.L1OC_CNo = np.zeros(cnoCount)

#%% Tracking engine
class TrackingEngine():
    """Perform GLONASS L1OC code and carrier tracking.
    ...
    def trkChannelsSerial(self, acqResults, corrEngine):
        """Perform GLONASS L1OC code and carrier tracking for selected channels.

        The channels are processed using channel-serial tracking mode. Each
        correlator call processes the L1OCd data and L1OCp pilot branches on the
        ...
        """
        from correlator import generateL1OCdCode, generateL1OCpBOCCode

        #--- Copy initial settings for all channels -----------------------
        settings = self.settings
        trkResults = self.trackResults
        # Determine how many acquired L1OC channels will be tracked.
        TrackedNr = min(sum(acqResults.acqFlag), settings.numberOfChannels)

        #%% Initialize tracking variables =================================
        # L1OCd is tracked on the equivalent return-zero half-chip grid.
        codeLength = settings.codeLength
        fineFactor = settings.L1OCFineFactor
        # Correlators use the common fine grid of the 4-period pilot code.
        ...
        corrSettings.L1OcpCodeLength = (
            settings.L1OcpCodeLength * fineFactor)
        ...
        # Number of preallocated coherent L1OCd code integrations.
        ...
        for channelNr in range(TrackedNr):
            # Mutable result container for the current L1OC channel.
            ...
            # Repeat the L1OCd branch on the fine grid and across the four
            # L1OCd periods spanned by one complete L1OCp code.
            L1OCdCode = generateL1OCdCode(
                chResults.PRN, settings)
            L1OCdCode = np.tile(
                np.repeat(L1OCdCode, fineFactor), 4)
            L1OCdCode = np.concatenate(
                ([L1OCdCode[-1]], L1OCdCode, [L1OCdCode[0]]))
            # L1OCp is already generated on the same fine TDM/BOC grid.
            L1OCpCode = generateL1OCpBOCCode(
                chResults.PRN, settings)
            L1OCpCode = np.concatenate(
                ([L1OCpCode[-1]], L1OCpCode, [L1OCpCode[0]]))
            L1OCCodeTable = np.concatenate((L1OCdCode, L1OCpCode))
            if settings.correlatorType == 1:
                L1OCCodeTable = np.asarray(L1OCCodeTable, dtype=np.int32)
            elif settings.correlatorType == 2:
                L1OCCodeTable = np.asarray(L1OCCodeTable, dtype=np.int8)
            ...
            # One-based L1OCp period aligned with the current L1OCd epoch.
            L1OCpCodePhase = int(acqResults.L1OCpCodePhase[channelNr])
            ...
            for loopCnt in range(numToProcess):
                ...
                # it for every coherent L1OC integration interval.
                ...
                # Record the absolute sample index of this L1OC code epoch.
                ...
                # Map the L1OCd residual phase into the corresponding phase
                # of the complete four-period L1OCp sequence and fine grid.
                remCodePhaseL1OC = (remCodePhase + codeLength *
                                    (L1OCpCodePhase - 1))
                corrRemCodePhase = remCodePhaseL1OC * fineFactor
                corrCodePhaseStep = codePhaseStep * fineFactor
                if settings.correlatorType == 0:
                    # NumPy reference QPSK correlator.
                    correValues = corrEngine(
                        corrSettings, rawSignal, L1OCCodeTable, remCarrPhase,
                ...
                remCarrPhase = np.fmod(
                    ...
                    2 * np.pi)
                # Advance and wrap the pilot-period phase after four L1OCd
                # coherent integrations.
                L1OCpCodePhase = L1OCpCodePhase % 4 + 1

                #%% Find PLL error and update carrier NCO ==================
                carrError = np.arctan(Q_P / I_P) / (2.0 * np.pi)
                if settings.pilotTRKflag:
                    # atan is unaffected by the L1OC pilot overlay symbols.
                    carrErrorP = (np.arctan(pilot_Q_P / pilot_I_P) /
                                  (2.0 * np.pi))
                ...
                if settings.pilotTRKflag:
                    codeErrorP = (pilotE - pilotL) / (pilotE + pilotL)
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # A 0.5-0.5 filter smooths data, pilot, and L1OC C/N0.
                    ...
                    chResults.L1OC_CNo[cnoCnt] = (CNoValue[2] * 0.5
                                                 + tempCNoValue[2] * 0.5)
    ...
    def trkChannelsParallel(self, acqResults, corrEngine):
        """Perform GLONASS L1OC tracking for all active channels.
        ...
        # Number of active L1OC channels to be tracked.
        ...
        # One-based pilot-period phase aligned with each current L1OCd epoch.
        L1OCpCodePhase = np.asarray(
            acqResults.L1OCpCodePhase[:channelCnt], dtype=np.int32).copy()
        ...
        # L1OCd code length on the equivalent return-zero half-chip grid.
        codeLength = settings.codeLength
        fineFactor = settings.L1OCFineFactor
        # Correlators use the common fine grid of the 4-period pilot code.
        corrSettings = copy(settings)
        ...
        #%% Generate L1OC code tables for acquired signals =================
        # Each row contains equal-length, guarded L1OCd and L1OCp branches.
        branchLength = settings.L1OcpCodeLength * fineFactor + 2
        L1OCCodeTable = np.empty(
        ...
        for channelNr in range(channelCnt):
            ...
            L1OCpCode = np.concatenate(
                ([L1OCpCode[-1]], L1OCpCode, [L1OCpCode[0]]))
            L1OCCodeTable[channelNr] = np.concatenate((L1OCdCode, L1OCpCode))
        ...
        # Number of coherent L1OC tracking intervals to process.
        numToProcess = trkResults[0].I_P.size
        ...
        # samplesPerCode property and the 500 L1OCd periods per second.
        ...
        for loopCnt in range(numToProcess):
            #%% GUI update -----------------------------------------------
            # Update the GUI periodically without repainting every L1OC
            # coherent integration interval.
            if ((loopCnt + 1) % barUprate == 0 or loopCnt + 1 == numToProcess):
                completedMs = round((loopCnt + 1) * settings.intTime * 1000)
                totalMs = round(numToProcess * settings.intTime * 1000)
            ...
            # Convert each L1OCd residual phase to the absolute phase of the
            # corresponding four-period pilot sequence and common fine grid.
            remCodePhaseL1OC = (remCodePhase + codeLength *
                                (L1OCpCodePhase - 1))
            corrRemCodePhase = remCodePhaseL1OC * fineFactor
            corrCodePhaseStep = codePhaseStep * fineFactor
            # All backends return a 12 x channelCnt matrix containing L1OCd
            # and L1OCp Early/Prompt/Late I/Q correlations.
            if settings.correlatorType == 0:
                # NumPy reference QPSK correlator.
                correValues = corrEngine(
                    corrSettings, rawSignal, L1OCCodeTable, remCarrPhase,
            ...
            for channelNr in range(channelCnt):
                ...
                # L1OCd and L1OCp Early, Prompt, and Late outputs.
                ...
                # to the next L1OC coherent integration interval.
                ...
                L1OCpCodePhase[channelNr] = (
                    L1OCpCodePhase[channelNr] % 4 + 1)
                ...
                #%% C/N0 calculation --------------------------------------
                # Periodically estimate data, pilot, and combined L1OC C/N0,
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # channel-serial L1OC tracking path.
                    ...
                    chResults.L1OC_CNo[cnoCnt] = (currentCNoValue[2] * 0.5
    ...
    def plotTracking(self):
        ...
        for channelNr, chResults in enumerate(self.trackResults):
            ...
            #--- L1OC navigation-message symbols -------------------------
    ...
    def trackingRun(self, acqResults):
        """Run GLONASS L1OC code and carrier tracking.
