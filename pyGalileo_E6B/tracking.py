...
--------------------------------
Galileo E6B code and carrier tracking.

"""

import numpy as np

from commUtils import (WaitBar, calcCNoPld, calcLoopCoef, calcLoopCoefCarr)
...
class Channel:
    ...
    The results contain data- and pilot-channel prompt outputs, absolute E6B
    ...
    def __init__(self, settings):
        ...
        self.lockFlag = False
        # Absolute input-sample index at the start of each E6B code epoch.
        ...
        self.E6B_CNo = np.zeros(cnoCount)

#%% Tracking engine
class TrackingEngine():
    """Perform Galileo E6B code and carrier tracking.
    ...
    def trkChannelsSerial(self, acqResults, corrEngine):
        """Perform Galileo E6B code and carrier tracking for selected channels.

        The channels are processed using channel-serial tracking mode. Each
        correlator call processes the E6B data and E6C pilot branches on the
        ...
        # Determine how many acquired E6B channels will be tracked.
        ...
        # Third-order carrier-loop filter coefficients.
        pf3, pf2, pf1 = calcLoopCoefCarr(settings)

        # Number of preallocated coherent E6B primary-code integrations.
        ...
        for channelNr in range(TrackedNr):
            # Mutable result container for the current E6B channel.
            ...
            # Get E6B and E6C codes sampled once per primary-code chip and
            # add one guard chip at each end for Early/Late indexing.
            E6BCode = generateDataCode(chResults.PRN)
            E6BCode = np.concatenate(([E6BCode[-1]], E6BCode, [E6BCode[0]]))
            E6CCode = generatePilotCode(chResults.PRN)
            E6CCode = np.concatenate(([E6CCode[-1]], E6CCode, [E6CCode[0]]))
            E6CodeTable = np.concatenate((E6BCode, E6CCode))
            if settings.correlatorType == 1:
                E6CodeTable = np.asarray(E6CodeTable, dtype=np.int32)
            elif settings.correlatorType == 2:
                E6CodeTable = np.asarray(E6CodeTable, dtype=np.int8)
            ...
            for loopCnt in range(numToProcess):
                ...
                # it for every coherent E6B integration interval.
                ...
                # Record the absolute sample index of this E6B code epoch.
                chResults.absoluteSample[loopCnt] = fid.tell() / bytesPerSample
                ...
                if settings.correlatorType == 0:
                    ...
                    correValues = corrEngine(
                        corrSettings, rawSignal, E6CodeTable, remCarrPhase,
                ...
                remCodePhase = ( blksize * codePhaseStep 
                                                + remCodePhase - codeLength)
                remCarrPhase = np.fmod(carrPhaseStep * blksize 
                                    + remCarrPhase, 2 * np.pi)

                #%% Find PLL error and update carrier NCO ==================
                # The E6C secondary-code sign does not affect atan(Q/I).
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # A 0.5-0.5 filter smooths data, pilot, and E6B C/N0.
                    ...
                    chResults.E6B_CNo[cnoCnt] = (CNoValue[2] * 0.5
                                                 + tempCNoValue[2] * 0.5)
    ...
    def trkChannelsParallel(self, acqResults, corrEngine):
        """Perform Galileo E6B tracking for all active channels.
        ...
        # Number of active E6B channels to be tracked.
        ...
        # Third-order carrier-loop filter memories.
        d2CarrError = np.zeros(channelCnt)
        dCarrError = np.zeros(channelCnt)
        ...
        #%% Initialize tracking variables =================================
        # DLL coherent integration interval [s].
        PDIcode = settings.intTime
        #--- DLL variables ------------------------------------------------
        ...
        # Primary E6B code length in chips.
        codeLength = settings.codeLength
        corrSettings = settings

        #%% Generate E6B code tables for acquired signals ================
        # Each row contains guarded E6B and E6C primary-code branches.
        branchLength = codeLength + 2
        E6CodeTable = np.empty(
        ...
        for channelNr in range(channelCnt):
            ...
            chResults.PRN = int(acqResults.PRN[channelNr])
            E6BCode = generateDataCode(chResults.PRN)
            E6BCode = np.concatenate(
                ([E6BCode[-1]], E6BCode, [E6BCode[0]]))
            E6CCode = generatePilotCode(chResults.PRN)
            E6CCode = np.concatenate(
                ([E6CCode[-1]], E6CCode, [E6CCode[0]]))
            E6CodeTable[channelNr] = np.concatenate((E6BCode, E6CCode))
        ...
        # Number of coherent E6B tracking intervals to process.
        numToProcess = trkResults[0].I_P.size
        ...
        # samplesPerCode property and the 1000 E6B codes per second.
        ...
        for loopCnt in range(numToProcess):
            #%% GUI update -----------------------------------------------
            # Update the GUI periodically without repainting every E6B
            # coherent integration interval.
            if ((loopCnt + 1) % barUprate == 0 or loopCnt + 1 == numToProcess):
                completedMs = round((loopCnt + 1) * settings.intTime * 1000)
                totalMs = round(numToProcess * settings.intTime * 1000)
            ...
            # All backends return a 12 x channelCnt matrix containing E6BI
            # and E6BQ Early/Prompt/Late I/Q correlations.
            if settings.correlatorType == 0:
                # NumPy reference QPSK correlator.
                correValues = corrEngine(
                    corrSettings, rawSignal, E6CodeTable, remCarrPhase,
            ...
            for channelNr in range(channelCnt):
                ...
                # E6B and E6C Early, Prompt, and Late outputs.
                ...
                # to the next E6B coherent integration interval.
                ...
                # Third-order PLL filter output and NCO correction [Hz].
                d2CarrError[channelNr] += carrError*pf3
                dCarrError[channelNr] += (
                    d2CarrError[channelNr]+carrError*pf2)
                carrNco = dCarrError[channelNr]+carrError*pf1
                ...
                #%% C/N0 calculation --------------------------------------
                # Periodically estimate data, pilot, and combined E6B C/N0,
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # channel-serial E6B tracking path.
                    ...
                    chResults.E6B_CNo[cnoCnt] = (currentCNoValue[2] * 0.5
    ...
    def plotTracking(self):
        ...
        for channelNr, chResults in enumerate(self.trackResults):
            ...
            #--- E6B navigation-message symbols --------------------------
    ...
    def trackingRun(self, acqResults):
        """Run Galileo E6B code and carrier tracking.
