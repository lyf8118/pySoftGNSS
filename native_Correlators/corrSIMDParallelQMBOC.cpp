/*================================================================================
 * Filename: corrSIMDParallelQMBOC.cpp
 * Description: AVX2 SIMD channel-parallel QMBOC tracking correlator exposed
 ...
 * corrValues      : persistent output array, 18 correlation values per channel
 ...
 *          int* caCodeTable          I   C-contiguous table
 *                                     [channelCnt][3 * codeLen]
 *          int codeLen               I   guarded code length of one branch
 ...
 * Return : persistent [channelCnt][18] array ordered as data, pilot BOC(1,1),
 *          and pilot BOC(6,1) IE,QE,IP,QP,IL,QL values; overwritten by the
	...
	int* chSampleSize, int isDataRead)
{
	...
	if (!initialized)
	{
		...
		/* For local sampling code buffers:
		   data, pilot BOC(1,1), and pilot BOC(6,1) Early/Prompt/Late. */
		...
		corrValues = (double*)checkedMalloc(
			sizeof(double) * 2 * LOCAL_CODE_COUNT * channelCnt, "corrValues");
		...
		if (iBasebandSignal == NULL || qBasebandSignal == NULL ||
			...
			localCode[8] == NULL || corrValues == NULL ||
	...
	{
		/* Select one channel row from the C-contiguous three-branch code table. */
		int index = chInd * 3 * codeLen;
		...
		// Generate data/pilot BOC(1,1)/pilot BOC(6,1) local code sequences
		...
		correlator(iBasebandSignal, qBasebandSignal, localCode, (mwSize)chSampleSize[chInd],
			corrValues + (2 * LOCAL_CODE_COUNT) * chInd);
		// Pilot BOC(1,1) branch correlator
		correlator(iBasebandSignal, qBasebandSignal, localCode+3, (mwSize)chSampleSize[chInd],
			corrValues + (2 * LOCAL_CODE_COUNT) * chInd + 6);
		// Pilot BOC(6,1) branch correlator
		correlator(iBasebandSignal, qBasebandSignal, localCode+6, (mwSize)chSampleSize[chInd],
			corrValues + (2 * LOCAL_CODE_COUNT) * chInd + 12);
	}
 ...
 * Args   : int     *caCode          I   concatenated QMBOC code table
 ...
 *                                     data/pilotBOC11/pilotBOC61 Early, Prompt and Late
 * return : None 
 *------------------------------------------------------------------------------*/
	...
	const int* startIndex, const int* chSampleSize)
{
	if (rawSignal == NULL || caCodeTable == NULL || remCarrPhase == NULL ||
		...
		printf("   Invalid input arguments for corrSIMDParallelQMBOC.\n");
		return 0;
	}
	return 1;
}
