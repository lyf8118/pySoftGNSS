 ...
 * Description: GPU-assisted single-channel QMBOC tracking correlator exposed
 ...
 *          earlyLateSpc   I   half early-late spacing on the 12x code grid
 *          rawSignal      I   int16 IF samples
 *          caCode         I   int8 guarded code table arranged as
 ...
 *          remCodePhase   I   residual code phase on the 12x code grid
 *          codePhaseStep  I   code phase increment per sample on the 12x grid
 *          PRN            I   current satellite PRN for device-code caching
 * Return : pointer to static [data E/P/L I/Q, pilotBOC11 E/P/L I/Q,
 *          pilotBOC61 E/P/L I/Q], or NULL when first-call validation, memory
 *          allocation, or CUDA processing fails. The pointer is overwritten
 *          by the next corrEngine() call.
 *------------------------------------------------------------------------------*/
    ...
    double codePhaseStep, int inPRN)
{
    ...
    if (!initialized)
    {
         ...
         * codeLen      : number of elements in one guarded-code branch
        ...
        CUDA_CHECK(cudaMalloc(&d_caCode, 3 * codeLen * sizeof(signed char)));
        CUDA_CHECK(cudaMemcpy(d_caCode, caCode, 3 * codeLen * sizeof(signed char),
     ...
     * corrValues is a static 18-element host array. Every successful call
     ...
     *-------------------------------------------------------- */
    if (PRN != inPRN) {
        CUDA_CHECK(cudaMemcpy(d_caCode, caCode, 3 * codeLen * sizeof(signed char), cudaMemcpyHostToDevice));
        PRN = inPRN;
    }
 ...
 *          const signed char* caCode I   three concatenated guarded int8 codes
 *          int codeLen               I   guarded-code elements in one branch
    ...
    const signed char* caCode, int codeLen)
{
    if (rawSignal == NULL || caCode == NULL || rawSignalLenIn <= 0 ||
        ...
        printf("   Invalid input arguments for corrGPUSerialQMBOC.\n");
        return 0;
    }
    return 1;
}
