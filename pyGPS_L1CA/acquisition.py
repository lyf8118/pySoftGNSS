...
-----------------------------------
GPS L1 C/A acquisition on CPU or GPU.
...
#%% Acquisition results
class AcqEngine():
    """Perform GPS L1 C/A acquisition and store its results.
    ...
    def __init__(self, settings):
        """Initialize result arrays through the largest configured GPS PRN.
        ...
        # Zero-based C/A-code phase [samples], shape = (svNum,); inf if unavailable.
    ...
    def acqProcessCPU(self):
        ...
        The function searches for GPS L1 C/A signals of all satellites listed
        ...
        # Convert interleaved I/Q samples into a complex signal. Subtracting
        # the mean first follows acquisitionCPU.m and removes the DC component.
        ...
        # Actual duration represented by one sampled C/A code period.
        ...
        # Map the complete 200 ms fine-acquisition interval to C/A-code chips
        ...
        for PRN in settings.acqSatelliteList:
            ...
            caCodeSamples = correlator.codeSampling(settings, PRN, samplesPerCode)
            ...
            if self.peakMetric[PRN-1] > settings.acqThreshold :
                ...
                # The original GPS L1 C/A code contains 1023 chips.
                caCode = correlator.generateCAcode(settings, PRN)
    ...
    def acqProcessGPU(self):
        ...
        # Number of samples in one nominal C/A code period.
        samplesPerCode = settings.samplesPerCode
        ...
        # a complex signal. Subtracting
        # the mean first follows acquisitionGPU.m and removes the DC component.
        longSignal = longSignal - np.mean(longSignal)
        if settings.fileType == 2:
            longSignal = longSignal[::2] + longSignal[1::2] * 1j
        ...
        #--- Variables for fine acquisition -------------------------------
        # Map all 200 ms absolute sample positions to C/A-code chips once. The
        ...
        for PRN in settings.acqSatelliteList:
            ...
            if self.peakMetric[PRN-1] > settings.acqThreshold :
                ...
                caCode = xp.asarray( correlator.generateCAcode(settings, PRN),
                                    dtype=realDataType)
