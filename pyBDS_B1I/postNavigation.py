...
--------------------------------------
BDS B1I/B2I D1/D2 navigation-message decoding and position calculation.
...
#%% BDS B1I/B2I broadcast ephemeris
class Ephemeris:
    """Keep and decode one BDS B1I/B2I D1 or D2 broadcast ephemeris.
    ...
    def __init__(self):
        ...
        self.URAI = None
        self.WN = None
        # Clock-data reference time and B1I group-delay correction.
        self.t_oc = None
        self.T_GD_1 = 0.0
    ...
    def decodeEphemeris(self, bits, PRN):
        ...
        ``bits`` must contain 1500 navigation bits and its first element must
        be the first bit of a subframe. The subframe ID of the first supplied
        subframe is not important. GEO satellites use the D2 message; IGSO
        and MEO satellites use the D1 message. BCH correction is performed
        while each subframe or page is decoded.
        ...
        if PRN <= 5 or 59 <= PRN <= 63:
            ...
            if bits.size < 15000:
                return np.inf
            # Fields split across adjacent D2 pages are accumulated here.
            fragments = {name: ["", ""] for name in (
                "a1", "C_uc", "e", "C_ic", "i_0", "omegaDot", "omega")}
            pageValid = np.zeros(10, dtype=bool)
            ...
            for pageIndex in range(50):
                ...
                if pageNumber == 1:
                    self.SatH1 = self.bin2dec(self._field(subframe, 47))
                    self.IODC = self.bin2dec(self._field(subframe, (48,52)))
                    self.URAI = self.bin2dec(self._field(subframe, (61,64)))
                    self.WN = self.bin2dec(self._field(subframe, (65,77)))
                    self.t_oc = self.bin2dec(self._field(subframe, (78,82), (91,102)))*2**3
                    self.T_GD_1 = twosComp2dec(self._field(subframe, (103,112)))*0.1e-9
                #--- Page 2: ionospheric model parameters ------------------
                elif pageNumber == 2:
                    self.alpha0 = twosComp2dec(self._field(subframe, (47,52), (61,62)))*2**-30
                    self.alpha1 = twosComp2dec(self._field(subframe, (63,70)))*2**-27
                    self.alpha2 = twosComp2dec(self._field(subframe, (71,78)))*2**-24
                    self.alpha3 = twosComp2dec(self._field(subframe, (79,82), (91,94)))*2**-24
                    self.beta0 = twosComp2dec(self._field(subframe, (95,102)))*2**11
                    self.beta1 = twosComp2dec(self._field(subframe, (103,110)))*2**14
                    self.beta2 = twosComp2dec(self._field(subframe, (111,112), (121,126)))*2**16
                    self.beta3 = twosComp2dec(self._field(subframe, (127,134)))*2**16
                #--- Pages 3 through 10: clock and ephemeris parameters ----
                elif pageNumber == 3:
                    self.a0 = twosComp2dec(self._field(subframe, (101,112), (121,132)))*2**-33
                    fragments["a1"][0] = self._field(subframe, (133,136))
                elif pageNumber == 4:
                    fragments["a1"][1] = self._field(subframe, (47,52), (61,72))
                    self.a2 = twosComp2dec(self._field(subframe, (73,82), 91))*2**-66
                    self.IODE = self.bin2dec(self._field(subframe, (92,96)))
                    self.deltan = twosComp2dec(self._field(subframe, (97,112)))*2**-43*bdsPi
                    fragments["C_uc"][0] = self._field(subframe, (121,134))
                elif pageNumber == 5:
                    fragments["C_uc"][1] = self._field(subframe, (47,50))
                    self.M_0 = twosComp2dec(self._field(subframe, (51,52), (61,82), (91,98)))*2**-31*bdsPi
                    self.C_us = twosComp2dec(self._field(subframe, (99,112), (121,124)))*2**-31
                    fragments["e"][0] = self._field(subframe, (125,134))
                elif pageNumber == 6:
                    fragments["e"][1] = self._field(subframe, (47,52), (61,76))
                    self.sqrtA = self.bin2dec(self._field(subframe, (77,82), (91,112), (121,124)))*2**-19
                    fragments["C_ic"][0] = self._field(subframe, (125,134))
                elif pageNumber == 7:
                    fragments["C_ic"][1] = self._field(subframe, (47,52), (61,62))
                    self.C_is = twosComp2dec(self._field(subframe, (63,80)))*2**-31
                    self.t_oe = self.bin2dec(self._field(subframe, (81,82), (91,105)))*2**3
                    fragments["i_0"][0] = self._field(subframe, (106,112), (121,134))
                elif pageNumber == 8:
                    fragments["i_0"][1] = self._field(subframe, (47,52), (61,65))
                    self.C_rc = twosComp2dec(self._field(subframe, (66,82), 91))*2**-6
                    self.C_rs = twosComp2dec(self._field(subframe, (92,109)))*2**-6
                    fragments["omegaDot"][0] = self._field(subframe, (110,112), (121,136))
                elif pageNumber == 9:
                    fragments["omegaDot"][1] = self._field(subframe, (47,51))
                    self.omega_0 = twosComp2dec(self._field(subframe, 52, (61,82), (91,99)))*2**-31*bdsPi
                    fragments["omega"][0] = self._field(subframe, (100,112), (121,134))
                elif pageNumber == 10:
                    fragments["omega"][1] = self._field(subframe, (47,51))
                    self.iDot = twosComp2dec(self._field(subframe, 52, (61,73)))*2**-43*bdsPi
                # Record successful decoding of every requisite D2 page.
                if 1 <= pageNumber <= 10:
                    pageValid[pageNumber-1] = True

            # Combine every split field in fixed MSB-page/LSB-page order.
            fieldBits = "".join(fragments["a1"])
            if len(fieldBits) == 22:
                self.a1 = twosComp2dec(fieldBits)*2**-50
            fieldBits = "".join(fragments["C_uc"])
            if len(fieldBits) == 18:
                self.C_uc = twosComp2dec(fieldBits)*2**-31
            fieldBits = "".join(fragments["e"])
            if len(fieldBits) == 32:
                self.e = self.bin2dec(fieldBits)*2**-33
            fieldBits = "".join(fragments["C_ic"])
            if len(fieldBits) == 18:
                self.C_ic = twosComp2dec(fieldBits)*2**-31
            fieldBits = "".join(fragments["i_0"])
            if len(fieldBits) == 32:
                self.i_0 = twosComp2dec(fieldBits)*2**-31*bdsPi
            fieldBits = "".join(fragments["omegaDot"])
            if len(fieldBits) == 24:
                self.omegaDot = twosComp2dec(fieldBits)*2**-43*bdsPi
            fieldBits = "".join(fragments["omega"])
            if len(fieldBits) == 32:
                self.omega = twosComp2dec(fieldBits)*2**-31*bdsPi

            # All ten pages are required for a complete GEO ephemeris.
            self.flag = int(np.all(pageValid))

        #%% Ephemeris for MEO and IGSO satellites ==========================
        elif 6 <= PRN <= 58:
            # D1 ephemeris requires subframes 1, 2 and 3.
            subframeValid = np.zeros(3, dtype=bool)
            toeMsb = toeLsb = ""
            # Tracking starts at an arbitrary subframe, so inspect all five.
            for frameIndex in range(5):
                ...
                if subframeID == 1:
                    self.SatH1 = self.bin2dec(self._field(subframe, 43))
                    self.IODC = self.bin2dec(self._field(subframe, (44,48)))
                    self.URAI = self.bin2dec(self._field(subframe, (49,52)))
                    self.WN = self.bin2dec(self._field(subframe, (61,73)))
                    self.t_oc = self.bin2dec(self._field(subframe, (74,82), (91,98)))*2**3
                    self.T_GD_1 = twosComp2dec(self._field(subframe, (99,108)))*0.1e-9
                    self.alpha0 = twosComp2dec(self._field(subframe, (127,134)))*2**-30
                    self.alpha1 = twosComp2dec(self._field(subframe, (135,142)))*2**-27
                    self.alpha2 = twosComp2dec(self._field(subframe, (151,158)))*2**-24
                    self.alpha3 = twosComp2dec(self._field(subframe, (159,166)))*2**-24
                    self.beta0 = twosComp2dec(self._field(subframe, (167,172), (181,182)))*2**11
                    self.beta1 = twosComp2dec(self._field(subframe, (183,190)))*2**14
                    self.beta2 = twosComp2dec(self._field(subframe, (191,198)))*2**16
                    self.beta3 = twosComp2dec(self._field(subframe, (199,202), (211,214)))*2**16
                    self.a2 = twosComp2dec(self._field(subframe, (215,225)))*2**-66
                    self.a0 = twosComp2dec(self._field(subframe, (226,232), (241,257)))*2**-33
                    self.a1 = twosComp2dec(self._field(subframe, (258,262), (271,287)))*2**-50
                    self.IODE = self.bin2dec(self._field(subframe, (288,292)))
                    subframeValid[0] = True
                #--- Subframe 2: first part of the broadcast ephemeris -----
                elif subframeID == 2:
                    self.deltan = twosComp2dec(self._field(subframe, (43,52), (61,66)))*2**-43*bdsPi
                    self.C_uc = twosComp2dec(self._field(subframe, (67,82), (91,92)))*2**-31
                    self.M_0 = twosComp2dec(self._field(subframe, (93,112), (121,132)))*2**-31*bdsPi
                    self.e = self.bin2dec(self._field(subframe, (133,142), (151,172)))*2**-33
                    self.C_us = twosComp2dec(self._field(subframe, (181,198)))*2**-31
                    self.C_rc = twosComp2dec(self._field(subframe, (199,202), (211,224)))*2**-6
                    self.C_rs = twosComp2dec(self._field(subframe, (225,232), (241,250)))*2**-6
                    self.sqrtA = self.bin2dec(self._field(subframe, (251,262), (271,290)))*2**-19
                    toeMsb = self._field(subframe, (291,292))
                    subframeValid[1] = True
                #--- Subframe 3: remaining broadcast ephemeris fields ------
                elif subframeID == 3:
                    toeLsb = self._field(subframe, (43,52), (61,65))
                    self.i_0 = twosComp2dec(self._field(subframe, (66,82), (91,105)))*2**-31*bdsPi
                    self.C_ic = twosComp2dec(self._field(subframe, (106,112), (121,131)))*2**-31
                    self.omegaDot = twosComp2dec(self._field(subframe, (132,142), (151,163)))*2**-43*bdsPi
                    self.C_is = twosComp2dec(self._field(subframe, (164,172), (181,189)))*2**-31
                    self.iDot = twosComp2dec(self._field(subframe, (190,202), 211))*2**-43*bdsPi
                    self.omega_0 = twosComp2dec(self._field(subframe, (212,232), (241,251)))*2**-31*bdsPi
                    self.omega = twosComp2dec(self._field(subframe, (252,262), (271,291)))*2**-31*bdsPi
                    subframeValid[2] = True

            # Reassemble the split ephemeris reference time.
            if len(toeMsb+toeLsb) == 17:
                self.t_oe = self.bin2dec(toeMsb+toeLsb)*2**3
            # All three requisite D1 subframes must have passed BCH decoding.
            self.flag = int(np.all(subframeValid))
        return SOW
    ...
    def NAVdecoding(cls, trackResult, settings):
        ...
        # Collect the 1500-bit interval needed by the ephemeris decoder.
        recordLength = 1500*20
        ...
        if PRN <= 5 or 59 <= PRN <= 63:
            ...
            demodulation = np.tile(nhCode, 1500)
    ...
    def satpos(self, transmitTime):
        ...
        # Apply the broadcast clock model and B1I group-delay correction.
...
class NavigationEngine:
    ...
    def navigationRun(self, trackResults):
        ...
        # B1I/B2I primary-code stream. Infinity means no frame was decoded.
