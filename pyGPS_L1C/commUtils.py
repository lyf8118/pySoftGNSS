...
def calcCNoPld(chResults, settings, loopCnt):
    ...
    ----
        ...
        settings   - object
                    Receiver settings.
        loopCnt    - int
                    Zero-based iteration index for the C/N0 calculation.
    Returns
    -------
        CNo         - numpy.ndarray
                    Data, pilot, and whole-signal L1C C/N0 in dB-Hz.
    ...
    # MATLAB uses the quadrature-rotated prompt orientation in the normal
    # L1C pilot-tracking mode.
    ...
    #--- C/N0 estimate for the whole L1C signal --------------------------
    CNo[2] = 10 * np.log10(dataCNo + pilotCNo)
    return CNo, pllDetector


#%% Composite L1C DLL weighting
def calcWeighingFactor(settings):
    """Calculate the data-channel weight for the composite code error.
    ...
    ----
        settings    - object
                    Receiver settings containing code frequency and
                    front-end bandwidth.
    Returns
    -------
        factor      - float
    ...
    # Power spectral density of the BOC(1,1) data channel.
    def G_BOC1_1f(f):
        x = f / fc
    ...
    # The TMBOC pilot PSD contains 29/33 BOC(1,1) and 4/33 BOC(6,1).
    def G_BOC6_1f(f):
        x = f / fc
...
def calculatePseudoranges(trackResults, subFrameStart, TOW, currMeasSample,
    ...
    ----
        ...
        subFrameStart - array-like
                      ...
                      L1C primary-code periods from the start of tracking.
        TOW           - array-like
...
def twosComp2dec(binaryNumber):
    ...
    if not isinstance(binaryNumber, str):
        binaryNumber = "".join(
            np.asarray(binaryNumber, dtype=str).reshape(-1))
