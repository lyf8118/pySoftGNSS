...
--------------------------------
Galileo E5a code and carrier tracking.
...
class Channel:
    ...
    The results contain data- and pilot-channel prompt outputs, absolute E5a
    ...
    def __init__(self, settings):
        ...
        self.lockFlag = False
        # Absolute input-sample index at the start of each E5a code epoch.
        ...
        self.E5a_CNo = np.zeros(cnoCount)

#%% Tracking engine
class TrackingEngine():
    """Perform Galileo E5a code and carrier tracking.
    ...
    def trkChannelsSerial(self, acqResults, corrEngine):
        """Perform Galileo E5a code and carrier tracking for selected channels.

        The channels are processed using channel-serial tracking mode. Each
        correlator call processes the E5aI data and E5aQ pilot branches on the
        ...
        # Determine how many acquired E5a channels will be tracked.
        ...
        # Number of preallocated coherent E5a primary-code integrations.
        ...
        for channelNr in range(TrackedNr):
            # Mutable result container for the current E5a channel.
            ...
            # Get E5aI and E5aQ codes sampled once per primary-code chip and
            # add one guard chip at each end for Early/Late indexing.
            E5aICode = generateDataCode(chResults.PRN)
            E5aICode = np.concatenate(
                ([E5aICode[-1]], E5aICode, [E5aICode[0]]))
            E5aQCode = generatePilotCode(chResults.PRN)
            E5aQCode = np.concatenate(
                ([E5aQCode[-1]], E5aQCode, [E5aQCode[0]]))
            E5aCodeTable = np.concatenate((E5aICode, E5aQCode))
            if settings.correlatorType == 1:
                E5aCodeTable = np.asarray(E5aCodeTable, dtype=np.int32)
            elif settings.correlatorType == 2:
                E5aCodeTable = np.asarray(E5aCodeTable, dtype=np.int8)
            ...
            for loopCnt in range(numToProcess):
                ...
                # it for every coherent E5a integration interval.
                ...
                # Record the absolute sample index of this E5a code epoch.
                ...
                if settings.correlatorType == 0:
                    ...
                    correValues = corrEngine(
                        corrSettings, rawSignal, E5aCodeTable, remCarrPhase,
                ...
                # E5aQ is pi/2 ahead of E5aI; rotate it back before combining.
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # A 0.5-0.5 filter smooths data, pilot, and E5a C/N0.
                    ...
                    chResults.E5a_CNo[cnoCnt] = (CNoValue[2] * 0.5
                                                 + tempCNoValue[2] * 0.5)
    ...
    def trkChannelsParallel(self, acqResults, corrEngine):
        """Perform Galileo E5a tracking for all active channels.
        ...
        # Number of active E5a channels to be tracked.
        ...
        # Primary E5a code length in chips.
        codeLength = settings.codeLength
        corrSettings = settings

        #%% Generate E5a code tables for acquired signals ================
        # Each row contains guarded E5aI and E5aQ primary-code branches.
        branchLength = codeLength + 2
        E5aCodeTable = np.empty(
        ...
        for channelNr in range(channelCnt):
            ...
            E5aQCode = np.concatenate(
                ([E5aQCode[-1]], E5aQCode, [E5aQCode[0]]))
            E5aCodeTable[channelNr] = np.concatenate((E5aICode, E5aQCode))
        ...
        # Number of coherent E5a tracking intervals to process.
        numToProcess = trkResults[0].I_P.size
        ...
        bytesPerSample = dataAdaptCoeff * sampleType.itemsize
        # Number of logical samples per E5a-code period and per second.
        ...
        for loopCnt in range(numToProcess):
            #%% GUI update -----------------------------------------------
            # Update the GUI periodically without repainting every E5a
            # coherent integration interval.
            if ((loopCnt + 1) % barUprate == 0 or loopCnt + 1 == numToProcess):
                completedMs = round((loopCnt + 1) * settings.intTime * 1000)
                totalMs = round(numToProcess * settings.intTime * 1000)
            ...
            # All backends return a 12 x channelCnt matrix containing E5aI
            # and E5aQ Early/Prompt/Late I/Q correlations.
            if settings.correlatorType == 0:
                # NumPy reference QPSK correlator.
                correValues = corrEngine(
                    corrSettings, rawSignal, E5aCodeTable, remCarrPhase,
            ...
            for channelNr in range(channelCnt):
                ...
                # E5aI and E5aQ Early, Prompt, and Late outputs.
                ...
                # to the next E5a coherent integration interval.
                ...
                #%% C/N0 calculation --------------------------------------
                # Periodically estimate data, pilot, and combined E5a C/N0,
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # channel-serial E5a tracking path.
                    ...
                    chResults.E5a_CNo[cnoCnt] = (currentCNoValue[2] * 0.5
    ...
    def plotTracking(self):
        ...
        for channelNr, chResults in enumerate(self.trackResults):
            ...
            #--- E5a navigation-message symbols --------------------------
    ...
    def trackingRun(self, acqResults):
        """Run Galileo E5a code and carrier tracking.
