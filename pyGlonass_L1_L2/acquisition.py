...
-----------------------------------
GLONASS L1/L2 acquisition on CPU or GPU.

"""

import numpy as np

#%% GLONASS acquisition results
class AcqEngine():
    """Perform GLONASS L1/L2 acquisition and store its results.
    ...
    def __init__(self, settings):
        """Initialize acquisition-result arrays for 14 frequency channels.
        ...
        # Fixed FDMA slots for K=-7...+6; this is not a PRN-indexed result set.
        svNum = 14
        # Refined carrier frequencies [Hz], shape (svNum,); inf means absent.
        self.carrFreq = np.full(svNum, np.inf)
        # Zero-based C/A-code phases [samples], shape (svNum,); inf means absent.
        ...
        # GLONASS FDMA channel K for each slot, ordered from -7 through +6,
        # shape ``(svNum,)``.
        self.K = np.arange(-7, 7, dtype=int)
    ...
    def acqProcessCPU(self):
        ...
        The function searches for GLONASS L1/L2 signals of all satellites listed
        ...
        -------
            None
                ...
                ``numpy.inf`` if a signal is not detected for a K.
        ...
        # Actual duration represented by one sampled GLONASS C/A-code period.
        codePeriod = samplesPerCode * ts
        # Number of samples per GLONASS C/A-code chip
        ...
        # Map the complete 200 ms fine-acquisition interval to GLONASS C/A-code chips
        # once. The index is independent of K and is reused for every signal.
        ...
        # Reuse these buffers for every K/frequency bin to avoid repeated
        # allocation of large 2 ms arrays in the inner search loops.
        caCodes2ms = np.zeros(samplesPerCode * 2, dtype=np.float64)
        results = np.empty(samplesPerCode * 2, dtype=np.float64)

        # Perform search for all listed K numbers ...
        print('   (',end="", flush=True)

        #%% Coarse acquisition =============================================
        for K in settings.acqSatelliteList:
            # Generate C/A codes and sample them according to the sampling freq.
            caCodeSamples = correlator.codeSampling(settings, K, samplesPerCode)
            caCodeSamples = np.asarray(caCodeSamples, dtype=np.float64)
            # Copy the 1 ms code into the reusable 2 ms zero-padded buffer.
            caCodes2ms[:samplesPerCode] = caCodeSamples
            #--- Perform DFT of C/A code ----------------------------------
            caCodeFreqDom = np.conjugate(fft.fft(caCodes2ms))
            ...
            self.codePhase[K+7] = codePhaseMax
            # Store the peak ratio used as the GLRT acquisition statistic.
            self.peakMetric[K+7] = peakMax / secondPeak
            # Store the searched K.
            self.K[K+7] = K

            #%% Fine carrier-frequency search =============================
            if self.peakMetric[K+7] > settings.acqThreshold :
                # A GLRT statistic above the threshold indicates that the
                # signal has been found.
                self.acqFlag[K+7] = True
                # Indicate K number of the detected signal
                ...
                # Generate the complete continuous 200 ms local-code replica in
                # one operation, following acquisitionCPU.m.
                # The common GLONASS L1/L2 code contains 511 chips.
                caCode = correlator.generateCAcode(settings, K)
                caCode200ms = caCode[codeValueIndex]
                ...
                # Wipe off the local code and carrier and integrate each of the
                # 200 code periods.
                #--- Integration for each of the 200 codes ----------------
                sumPerCode = np.empty(200, dtype=np.complex128)

                for codePeriodIndex in range(200):
                    signalIndex = slice(codePeriodIndex * samplesPerCode,
                                        (codePeriodIndex + 1) * samplesPerCode)
                    localCodeCarr = (caCode200ms[signalIndex] *
                                     localCarr200cm[signalIndex])
                    sumPerCode[codePeriodIndex] = np.dot(
                                      sig200ms[codePeriodIndex], localCodeCarr)

                #--- Find the fine carrier frequency ----------------------
                # Index of the max power
                maxPowerIndex = np.abs(fft.fft(sumPerCode**2)).argmax()
                # FFT shift angle
                shiftAngle = np.angle(np.exp( -2*np.pi*1j* maxPowerIndex/200 ))/2
                self.carrFreq[K+7] = freqMax - shiftAngle/codePeriod/2/np.pi
                # Remove the FDMA channel offset before carrier-to-code
                # Doppler scaling, using this channel's RF carrier frequency.
                self.codeFreq[K+7] = settings.codeFreqBasis + (
                    (self.carrFreq[K+7] -
                     (settings.IF + settings.freqSpacing*K)) /
                    (settings.carrFreqBasis + settings.freqSpacing*K) *
                    settings.codeFreqBasis)
                # If IF is zero, use 1 Hz so the channel remains active.
                if self.carrFreq[K+7] == 0:
                    self.carrFreq[K+7] = 1
            else:
                #--- No signal with this K ----------------------------------
                print('. ',end="", flush=True)
    ...
    def acqProcessGPU(self):
        ...
        # Number of samples in one nominal GLONASS C/A-code period.
        samplesPerCode = settings.samplesPerCode
        ...
        # Pass the IF data to GPU memory. For complex input, first remove the
        # scalar-stream mean and combine the interleaved I/Q samples.
        if settings.fileType == 2:
            longSignal = longSignal - np.mean(longSignal)
            longSignal = longSignal[::2] + longSignal[1::2] * 1j
        ...
        #--- Variables for fine acquisition -------------------------------
        # Map all 200 ms absolute sample positions to GLONASS C/A-code chips once. The
        ...
        # Reuse the large 2 ms GPU buffers for every K/frequency bin.
        caCodes2ms = xp.zeros(samplesPerCode * 2, dtype=realDataType)
        results = xp.empty(samplesPerCode * 2, dtype=realDataType)

        # Search every configured satellite K.
        print('   (',end="", flush=True)

        #%% Coarse acquisition ============================================
        for K in settings.acqSatelliteList:
            # Generate one C/A period and sample it at the input sample rate.
            caCodeSamples = correlator.codeSampling(settings, K, samplesPerCode)
            caCodeSamples = xp.asarray(caCodeSamples, dtype=realDataType)
            # Zero-pad the 1 ms code to match the 2 ms signal block.
            caCodes2ms[:samplesPerCode] = caCodeSamples
            #--- Perform DFT of C/A code ----------------------------------
            # Conjugated code spectrum for frequency-domain correlation.
            caCodeFreqDom = xp.conjugate(fftBackend.fft(caCodes2ms))
            ...
            for freqBinIndex in range(numberOfFreqBins):
                ...
                for nonCohIndex in range(settings.acqNonCohTime):
                    ...
                    convCodeIQ = IQfreqDom * caCodeFreqDom
                    # Accumulate correlation magnitudes non-coherently.
            ...
            if self.peakMetric[K+7] > settings.acqThreshold :
                ...
                # Keep one unsampled 1023-chip C/A period on the GPU.
                caCode = xp.asarray( correlator.generateCAcode(settings, K),
                                    dtype=realDataType)

                # Generate the complete continuous 200 ms code replica in one
                # operation. Its values are only +/-1, so float32 is exact.
                caCode200ms = caCode[codeValueIndex]
                caCode200ms = caCode200ms.reshape((200, samplesPerCode))
                ...
                #--- Integration for each of the 200 codes ----------------
                sumPerCode = xp.empty(200, dtype=complexDataType)

                for codePeriodIndex in range(200):
                    # Combine this millisecond of the continuous code with the
                    # reusable one-millisecond carrier replica.
                    localCodeCarr = (caCode200ms[codePeriodIndex] * localCarr1ms)
                    ...
                    sumPerCode[codePeriodIndex] = xp.dot(sig200ms[codePeriodIndex],
                                                 localCodeCarr) * initialCarr
                ...
                maxPowerIndex = xp.abs(fftBackend.fft(sumPerCode**2)).argmax().item()
    ...
    def plotAcq(self):
        ...
        validK = np.isin(self.K, settings.acqSatelliteList)
        acqMask = validK & (self.carrFreq != np.inf)
        notAcqMask = validK & (self.carrFreq == np.inf)
        #%% Plot all results ==============================================
        fig, ax = plt.subplots()
        ax.bar(self.K[acqMask], self.peakMetric[acqMask], color = 'b', width = 0.85)
        ax.bar(self.K[notAcqMask], self.peakMetric[notAcqMask], color = 'r', width = 0.85)
        ax.legend(labels=['Acquired signals', 'Not acquired signals'])
        ax.set_xlabel('Frequency channel K (no bar - channel is not searched)')
        ax.set_ylabel('Acquisition Metric')
        ax.set_title('GLONASS acquisition results')
        plt.grid(True)
        plt.xticks(np.sort(self.K[validK]))
        plt.tight_layout()
        plt.show()
        return
    ...
    def preRun(self):
        ...
        Kindexes = self.peakMetric.argsort()[::-1]
        self.peakMetric = self.peakMetric.take(Kindexes)
        self.carrFreq = self.carrFreq.take(Kindexes)
        self.codePhase = self.codePhase.take(Kindexes)
        self.K = self.K.take(Kindexes)
        self.acqFlag =  self.acqFlag.take(Kindexes)
        self.codeFreq =  self.codeFreq.take(Kindexes)
    ...
    def showChannelStatus(self):
        ...
        settings = self._settings
        print('\n*=========*=====*===============*===========*=============*=========*')
        print('| Channel | Frequency channel K |   Frequency   |  Doppler  | Code Offset | Status  |')
        print('*=========*=====*===============*===========*=============*========*')
        for channelNr in range(min(settings.numberOfChannels, self.K.size)):
            if self.acqFlag[channelNr]:
                print('|    %2d   | %3d |  %2.5e  |   %5.0f   |    %6d   |    %1s    |' % (
                    channelNr,
                    self.K[channelNr],
                    self.carrFreq[channelNr],
                    self.carrFreq[channelNr] - (settings.IF + settings.freqSpacing * self.K[channelNr]),
