...
--------------------------------
GLONASS L1/L2 code and carrier tracking.
...
class TrackingEngine():
    """Perform GLONASS L1/L2 code and carrier tracking.
    ...
    def trkChannelsSerial(self, acqResults, corrEngine):
        ...
        ----
            acqResults - acquisition.AcqEngine
                       frequency channels, carrier frequencies, and code phases of all
        ...
        """
        from correlator import generateCAcode
        ...
        for channelNr in range(TrackedNr):
            ...
            # a GLONASS frequency channel was assigned.
            if acqResults.acqFlag[channelNr] != True:
                continue

            # Save additional information: the K tracked by this channel.
            chResults.K = acqResults.K[channelNr]
            ...
            # Generate one bipolar GLONASS C/A-code value per chip.
            codeChips = generateCAcode(settings,chResults.K)
            ...
            for loopCnt in range(codePeriods):
                ...
                if ((loopCnt + 1) % barUprate == 0 or
                    ...
                    trackingStatus = (
                        f'Ch {channelNr + 1}/{TrackedNr}, K {chResults.K}\n'
    ...
    def trkChannelsParallel(self, acqResults, corrEngine):
        ...
        # Absolute zero-based sample of the next GLONASS C/A-code start in the record.
        ...
        # Each row contains [last chip, one GLONASS C/A period, first chip].
        caCodeTable = np.empty((channelCnt, settings.codeLength + 2),
                               dtype=np.float64)

        for channelNr in range(channelCnt):
            # Result container and K for the current active channel.
            chResults = trkResults[channelNr]
            chResults.K = int(acqResults.K[channelNr])
            # Generate one GLONASS C/A-code value per chip.
            caCode = generateCAcode(settings, chResults.K)
        ...
        #%% Initialize tracking variables =================================
        # Number of code periods to process; one GLONASS C/A period is 1 ms.
        ...
        # Number of logical samples per GLONASS C/A-code period and per second.
        ...
        for loopCnt in range(codePeriods):
            ...
            if ((loopCnt + 1) % barUprate == 0 or
                ...
                trackingStatus = (
                    f'Tracking {channelCnt} frequency channels\n'
                    f'1st K: {trkResults[0].K}\n'
                    f'{loopCnt + 1}/{codePeriods} ms, '
    ...
    def plotTracking(self):
        ...
        for channelNr, chResults in enumerate(self.trackResults):
            ...
            fig.suptitle(
                f'Channel {channelNr + 1} - K {chResults.K}')
        ...
        try:
            ...
            #--- Channel-parallel tracking --------------------------------
                ...
                if settings.correlatorType == 0:
                    ...
                    # GPU correlator
                    corrEngine = correlator.CorrGPUParallelBPSK()
