...
--------------------------------------
GPS L2C CNAV decoding and position calculation.
...
class Ephemeris:
    ...
    def decodeEphemeris(self, navBitsBin):
        ...
        # For message contents, refer to IS-GPS-200.
        ...
        if messageID == 10:
            ...
            if messageID == 30:
                ...
                self.ISC_L2C = twosComp2dec(navBitsBin[153:166])*2**-35
        ...
        # 12-second message duration to obtain the start of this message.
        TOW = self.bin2dec(navBitsBin[20:37])*6-12
        return TOW

    @classmethod
    def NAVdecoding(cls, I_P_InputBits):
        """Decode GPS L2C CNAV messages in one channel's tracking stream.

        The method performs hard-decision Viterbi decoding, preamble search,
        CRC-24Q checking, message-by-message ephemeris decoding, and
        extraction of the first message TOW.
        ...
        -------
            ...
            firstSubFrame - int or float
                          ...
                          tracking stream, expressed as the number of 20 ms
                          L2C CM-code periods since tracking began. It is
        ...
        # Take an even number of input symbols for convolutional decoding,
        # threshold the tracking outputs, and convert them to zero and one.
        dataBits = np.asarray(I_P_InputBits, dtype=np.float64)
    ...
    def satpos(self, transmitTime):
        ...
        # omitted from the PVT calculation. Refer to IS-GPS-200 for the
        # T_GD and ISC_L2C definitions.
        if self.idValid[2] == 30:
            satClkCorr = satClkCorr-self.T_GD+self.ISC_L2C
...
class NavigationEngine:
    ...
    def navigationRun(self, trackResults):
        ...
        # CNAV message types 10, 11, and one clock message from 30--37 are
        # required. Under the current broadcast sequence, these requisite
        # data may span four 12 s message slots because other CNAV messages
        # can be interleaved. Tracking may also start midway through the
        # first message, so at least 60 s of tracked data are required.
        if settings.msToProcess < 48000:
        ...
        # Starting position of the first valid message in the sequence of
        # 20 ms L2C CM-code tracking epochs. Infinity indicates that no valid
        ...
        for channelNr in activeChnList.copy():
            ...
            if (currentEph.idValid[0] != 10
                    ...
                    or not clockReady):
                print(f"    Requisite CNAV messages for PRN {PRN:02d} "
                      "were not all decoded.")
