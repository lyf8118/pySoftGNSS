...
--------------------------------
GPS L2C code and carrier tracking.
...
class Channel:
    ...
    The results contain data- and pilot-channel prompt outputs, absolute L2C
    ...
    def __init__(self, settings):
        ...
        self.lockFlag = False
        # Absolute input-sample index at the start of each L2C code epoch.
        ...
        self.L2C_CNo = np.zeros(cnoCount)

#%% Tracking engine
class TrackingEngine():
    """Perform GPS L2C code and carrier tracking.
    ...
    def trkChannelsSerial(self, acqResults, corrEngine):
        """Perform GPS L2C code and carrier tracking for selected channels.

        The channels are processed using channel-serial tracking mode. Each
        correlator call processes the CM data and CL pilot branches on the
        ...
        """
        from correlator import generateCMcode, generateCLcode

        #--- Copy initial settings for all channels -----------------------
        settings = self.settings
        trkResults = self.trackResults
        # Determine how many acquired L2C channels will be tracked.
        TrackedNr = min(sum(acqResults.acqFlag), settings.numberOfChannels)

        #%% Initialize tracking variables =================================
        # CM and CL use the primary-code chip grid.
        ...
        # Number of preallocated coherent L2C primary-code integrations.
        ...
        for channelNr in range(TrackedNr):
            # Mutable result container for the current L2C channel.
            ...
            # Repeat the CM data code to the 1.5 s CL-code length so both
            # branches use the same time-multiplexed QPSK table layout.
            cmCode = generateCMcode(settings, chResults.PRN)
            cmCode = np.tile(cmCode, 75)
            cmCode = np.concatenate(([cmCode[-1]], cmCode, [cmCode[0]]))
            clCode = generateCLcode(settings, chResults.PRN)
            clCode = np.concatenate(([clCode[-1]], clCode, [clCode[0]]))
            L2CCodeTable = np.concatenate((cmCode, clCode))
            if settings.correlatorType == 1:
                L2CCodeTable = np.asarray(L2CCodeTable, dtype=np.int32)
            elif settings.correlatorType == 2:
                L2CCodeTable = np.asarray(L2CCodeTable, dtype=np.int8)
            ...
            remCodePhase = 0.0
            remCarrPhase = 0.0
            # One-based CM-period index within the 75-period CL sequence.
            CLCodePhase = int(acqResults.CLCodePhase[channelNr])
            ...
            for loopCnt in range(numToProcess):
                ...
                # it for every coherent L2C integration interval.
                ...
                # Record the absolute sample index of this L2C code epoch.
                ...
                # Convert the residual phase within the current CM interval
                # to its absolute phase in the 75-period CL sequence.
                remCodePhaseL2C = (
                    remCodePhase + codeLength * (CLCodePhase - 1))
                if settings.correlatorType == 0:
                    # NumPy reference QPSK correlator.
                    correValues = corrEngine(
                        corrSettings, rawSignal, L2CCodeTable, remCarrPhase,
                        carrPhaseStep, remCodePhaseL2C, codePhaseStep)
                ...
                remCodePhase = np.remainder(
                    blksize * codePhaseStep + remCodePhaseL2C,
                    codeLength)
                CLCodePhase += 1
                if CLCodePhase >= 76:
                    CLCodePhase = 1
                ...
                # Average the CM-data and CL-pilot prompt phase errors.
                carrErrorQ = (np.arctan(pilot_Q_P / pilot_I_P) /
                              (2.0 * np.pi))
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # A 0.5-0.5 filter smooths data, pilot, and L2C C/N0.
                    ...
                    chResults.L2C_CNo[cnoCnt] = (CNoValue[2] * 0.5
                                                 + tempCNoValue[2] * 0.5)
    ...
    def trkChannelsParallel(self, acqResults, corrEngine):
        """Perform GPS L2C tracking for all active channels.
        ...
        # Number of active L2C channels to be tracked.
        ...
        # Fixed acquired L2C code-frequency bases for the loop update [chips/s].
        codeFreqBasis = codeFreq.copy()
        # Residual primary-code phase [chips].
        remCodePhase = np.zeros(channelCnt)
        # One-based CM-period indices within the 75-period CL sequences.
        CLCodePhase = np.asarray(
            acqResults.CLCodePhase[:channelCnt], dtype=np.int32).copy()
        ...
        # Primary L2C code length and Early/Late spacing in primary chips.
        codeLength = settings.codeLength
        corrSettings = settings

        #%% Generate L2C code tables for acquired signals =================
        # Each row contains equal-length guarded CM and CL branches.
        branchLength = settings.CLCodeLength + 2
        L2CCodeTable = np.empty(
        ...
        for channelNr in range(channelCnt):
            ...
            clCode = generateCLcode(settings, chResults.PRN)
            clCode = np.concatenate(([clCode[-1]], clCode, [clCode[0]]))
            L2CCodeTable[channelNr] = np.concatenate((cmCode, clCode))
        ...
        # Number of coherent L2C tracking intervals to process.
        numToProcess = trkResults[0].I_P.size
        ...
        # samplesPerCode property and the 50 CM-code periods per second.
        ...
        for loopCnt in range(numToProcess):
            #%% GUI update -----------------------------------------------
            # Update the GUI periodically without repainting every L2C
            # coherent integration interval.
            if ((loopCnt + 1) % barUprate == 0 or loopCnt + 1 == numToProcess):
                completedMs = round((loopCnt + 1) * settings.intTime * 1000)
                totalMs = round(numToProcess * settings.intTime * 1000)
            ...
            # Convert each CM residual phase to the corresponding phase in
            # its 75-period CL sequence before correlation.
            remCodePhaseL2C = (
                remCodePhase + codeLength * (CLCodePhase - 1))
            # All backends return a 12 x channelCnt matrix containing CM
            # and CL Early/Prompt/Late I/Q correlations.
            if settings.correlatorType == 0:
                # NumPy reference QPSK correlator.
                correValues = corrEngine(
                    corrSettings, rawSignal, L2CCodeTable, remCarrPhase,
                    carrPhaseStep, remCodePhaseL2C, codePhaseStep, startIdx,
            ...
            for channelNr in range(channelCnt):
                ...
                # CM and CL Early, Prompt, and Late outputs.
                ...
                # to the next L2C coherent integration interval.
                chResults.remCodePhase[loopCnt] = remCodePhase[channelNr]
                chResults.remCarrPhase[loopCnt] = remCarrPhase[channelNr]
                remCodePhase[channelNr] = np.remainder(
                    chSampSize[channelNr] * codePhaseStep[channelNr]
                    + remCodePhaseL2C[channelNr], codeLength)
                CLCodePhase[channelNr] += 1
                if CLCodePhase[channelNr] >= 76:
                    CLCodePhase[channelNr] = 1
                ...
                #%% C/N0 calculation --------------------------------------
                # Periodically estimate data, pilot, and combined L2C C/N0,
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # channel-serial L2C tracking path.
                    ...
                    chResults.L2C_CNo[cnoCnt] = (currentCNoValue[2] * 0.5
    ...
    def plotTracking(self):
        ...
        for channelNr, chResults in enumerate(self.trackResults):
            ...
            #--- L2C navigation-message symbols ---------------------------
    ...
    def trackingRun(self, acqResults):
        """Run GPS L2C code and carrier tracking.
