...
--------------------------------
BDS-3 B1C code and carrier tracking.
...
class Channel:
    ...
    The results contain data- and pilot-channel prompt outputs, absolute B1C
    ...
    def __init__(self, settings):
        ...
        self.lockFlag = False
        # Absolute input-sample index at the start of each B1C code epoch.
        ...
        self.B1C_CNo = np.zeros(cnoCount)

#%% Tracking engine
class TrackingEngine():
    """Perform BDS-3 B1C code and carrier tracking.
    ...
    def trkChannelsSerial(self, acqResults, corrEngine):
        """Perform BDS-3 B1C code and carrier tracking for selected channels.

        The channels are processed using channel-serial tracking mode. Each
        correlator call processes the data BOC(1,1), pilot BOC(1,1), and pilot
        BOC(6,1) branches on the common 12-times-primary-code grid.
        ...
        """
        from correlator import (generateDataBOC11, generatePilotBOC11,
                                generatePilotBOC61)

        #--- Copy initial settings for all channels -----------------------
        settings = self.settings
        trkResults = self.trackResults
        # Determine how many acquired B1C channels will be tracked.
        ...
        # Early-late spacing in primary-code chips for the DLL discriminator.
        earlyLateSpc = settings.dllCorrelatorSpacing
        # MATLAB receives settings by value. Use a shallow Python copy so the
        # correlator sees the 12x grid without changing receiver settings.
        corrSettings = copy(settings)
        corrSettings.dllCorrelatorSpacing = earlyLateSpc * 12
        ...
        # Weighting factors used to combine data- and pilot-channel errors.
        if settings.fullBandEn == 1:
            codeFactor = calcWeighingFactor(settings)
            carrFactor = 1 / 4
            pilotBOC61Factor = np.sqrt(4 / 33)
            pilotBOC11Factor = np.sqrt(29 / 33)
        else:
            codeFactor = 11 / 40
            carrFactor = 11 / 40

        # Number of preallocated coherent B1C primary-code integrations.
        ...
        if settings.correlatorType == 0:
            # NumPy reference QMBOC correlator.
            barUprate = 5
        else:
        ...
        for channelNr in range(TrackedNr):
            # Mutable result container for the current B1C channel.
            ...
            # Get the B1C data-channel BOC(1,1) waveform on the 12x grid and
            # add one guard value at each end.
            B1CData = generateDataBOC11(settings, chResults.PRN)
            B1CData = np.concatenate(([B1CData[-1]], B1CData, [B1CData[0]]))

            # Get the pilot BOC(1,1) and BOC(6,1) spreading waveforms and
            # add the same guard values required by Early/Late indexing.
            pilotBOC11 = generatePilotBOC11(settings, chResults.PRN)
            pilotBOC11 = np.concatenate(([pilotBOC11[-1]], 
                                         pilotBOC11, [pilotBOC11[0]]))
            pilotBOC61 = generatePilotBOC61(settings, chResults.PRN)
            pilotBOC61 = np.concatenate(([pilotBOC61[-1]], 
                                         pilotBOC61, [pilotBOC61[0]]))

            # Store the three branches in the shared QMBOC table layout.
            B1CCodeTable = np.concatenate((B1CData, pilotBOC11, pilotBOC61))
            if settings.correlatorType == 1:
                B1CCodeTable = np.asarray(B1CCodeTable, dtype=np.int32)
            elif settings.correlatorType == 2:
                B1CCodeTable = np.asarray(B1CCodeTable, dtype=np.int8)
            ...
            for loopCnt in range(numToProcess):
                ...
                # it for every coherent B1C integration interval.
                ...
                # Record the absolute sample index of this B1C code epoch.
                ...
                # The correlator code phase uses the 12x local-code grid.
                remCodePhase12 = remCodePhase * 12
                codePhaseStep12 = codePhaseStep * 12

                if settings.correlatorType == 0:
                    # NumPy reference QMBOC correlator.
                    correValues = corrEngine(corrSettings, rawSignal,
                                     B1CCodeTable,remCarrPhase, carrPhaseStep, 
                                     remCodePhase12,codePhaseStep12)
                elif settings.correlatorType == 1:
                    # CPU SIMD DLL QMBOC correlator.
                    correValues = corrEngine.corrEngine(
                        corrSettings, rawSignal, B1CCodeTable,
                        remCarrPhase, carrPhaseStep, remCodePhase12,
                        codePhaseStep12)
                elif settings.correlatorType == 2:
                    # CUDA DLL QMBOC correlator.
                    correValues = corrEngine.corrEngine(
                        corrSettings, rawSignal, B1CCodeTable,
                        remCarrPhase, carrPhaseStep, remCodePhase12,
                        codePhaseStep12, chResults.PRN)

                # All backends return data BOC(1,1), pilot BOC(1,1), and
                # pilot BOC(6,1) Early/Prompt/Late I/Q values in one vector.
                # Data branch: Early, Prompt, Late, each with I/Q.
                I_E, Q_E, I_P, Q_P, I_L, Q_L = correValues[:6]
                
                # Pilot BOC(1,1) branch.
                (p11_I_E, p11_Q_E, p11_I_P, p11_Q_P,
                                     p11_I_L, p11_Q_L) = correValues[6:12]

                if settings.fullBandEn == 1:
                    # Pilot BOC(6,1) branch.
                    (p61_I_E, p61_Q_E, p61_I_P, p61_Q_P,
                     p61_I_L, p61_Q_L) = correValues[12:18]

                    # Composite whole-pilot-channel correlations.
                    p_I_E = (-pilotBOC61Factor * p61_I_E 
                             + pilotBOC11Factor * p11_Q_E)
                    
                    p_Q_E = (-pilotBOC61Factor * p61_Q_E
                             - pilotBOC11Factor * p11_I_E)
                    
                    p_I_P = (-pilotBOC61Factor * p61_I_P
                             + pilotBOC11Factor * p11_Q_P)
                    
                    p_Q_P = (-pilotBOC61Factor * p61_Q_P
                             - pilotBOC11Factor * p11_I_P)
                    
                    p_I_L = (-pilotBOC61Factor * p61_I_L
                             + pilotBOC11Factor * p11_Q_L)
                    
                    p_Q_L = (-pilotBOC61Factor * p61_Q_L
                             - pilotBOC11Factor * p11_I_L)
                ...
                if settings.fullBandEn == 0:
                    # The B1C pilot is pi/2 ahead of the data channel.
                    p_QI = ((p11_I_P + 1j * p11_Q_P)* np.exp(-1j * np.pi / 2))
                    pCarrError = (np.arctan(np.imag(p_QI) / np.real(p_QI))                        
                                  / (2.0 * np.pi))
                elif settings.fullBandEn == 1:
                    pCarrError = (np.arctan(p_Q_P / p_I_P)/ (2.0 * np.pi))

                # Composite data/pilot carrier discriminator.
                ...
                if settings.fullBandEn == 0:
                    pE = np.sqrt(p11_I_E ** 2 + p11_Q_E ** 2)
                    pL = np.sqrt(p11_I_L ** 2 + p11_Q_L ** 2)
                elif settings.fullBandEn == 1:
                    pE = np.sqrt(p_I_E ** 2 + p_Q_E ** 2)
                    pL = np.sqrt(p_I_L ** 2 + p_Q_L ** 2)
                pCodeError = (pE - pL) / (pE + pL) * (1 - earlyLateSpc)

                E = np.sqrt(I_E ** 2 + Q_E ** 2)
                L = np.sqrt(I_L ** 2 + Q_L ** 2)
                codeError = (E - L) / (E + L) * (1 - earlyLateSpc)
                # Composite data/pilot code discriminator.
                codeError = (codeError * codeFactor + 
                             pCodeError * (1 - codeFactor))
                ...
                if settings.fullBandEn == 0:
                    chResults.Pilot_I_P[loopCnt] = p11_I_P
                    chResults.Pilot_Q_P[loopCnt] = p11_Q_P
                elif settings.fullBandEn == 1:
                    chResults.Pilot_I_P[loopCnt] = p_I_P
                    chResults.Pilot_Q_P[loopCnt] = p_Q_P

                #%% C/N0 calculation ======================================
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    CNoValue, pllDetector = calcCNoPldQmboc(chResults, 
                                                            settings, loopCnt)
                    
                    cnoCnt = (loopCnt + 1)// settings.CNoVSMinterval - 1
                    
                    # A 0.5-0.5 filter smooths data, pilot, and B1C C/N0.
                    ...
                    chResults.B1C_CNo[cnoCnt] = (CNoValue[2] * 0.5
                                                 + tempCNoValue[2] * 0.5)
    ...
    def trkChannelsParallel(self, acqResults, corrEngine):
        """Perform BDS-3 B1C tracking for all active channels.
        ...
        # Number of active B1C channels to be tracked.
        ...
        # Weighting factors for composite data/pilot DLL and PLL errors.
        if settings.fullBandEn == 1:
            codeFactor = calcWeighingFactor(settings)
            carrFactor = 1 / 4
            pilotBOC61Factor = np.sqrt(4 / 33)
        ...
        # Primary B1C code length and Early/Late spacing in primary chips.
        codeLength = settings.codeLength
        earlyLateSpc = settings.dllCorrelatorSpacing
        # Supply the correlator with spacing on its 12x local-code grid
        # without mutating the receiver-owned Settings instance.
        corrSettings = copy(settings)
        corrSettings.dllCorrelatorSpacing = earlyLateSpc * 12

        #%% Generate B1C code tables for acquired signals =================
        # Each row contains three guarded 12x-grid branches:
        # [data BOC(1,1), pilot BOC(1,1), pilot BOC(6,1)].
        branchLength = codeLength * 12 + 2
        B1CCodeTable = np.empty((channelCnt, branchLength * 3), dtype=np.float64)
        ...
        for channelNr in range(channelCnt):
            ...
            # Data-channel BOC(1,1) waveform with guard values.
            B1CData = generateDataBOC11(
                settings, chResults.PRN)
            B1CData = np.concatenate(([B1CData[-1]], B1CData, [B1CData[0]]))
            
            # Pilot BOC(1,1) waveform with guard values.
            pilotBOC11 = generatePilotBOC11(settings, chResults.PRN)
            pilotBOC11 = np.concatenate(
                            ([pilotBOC11[-1]], pilotBOC11, [pilotBOC11[0]]))
           
            # Pilot BOC(6,1) waveform with guard values.
            pilotBOC61 = generatePilotBOC61(
                settings, chResults.PRN)
            pilotBOC61 = np.concatenate(
                            ([pilotBOC61[-1]], pilotBOC61, [pilotBOC61[0]]))

            # Store the three guarded branches in the common QMBOC layout.
            B1CCodeTable[channelNr] = np.concatenate(
                                    (B1CData, pilotBOC11, pilotBOC61))

        if settings.correlatorType == 1:
            # SIMD shares an int32 QMBOC code table across all channels.
            B1CCodeTable = np.asarray(B1CCodeTable, dtype=np.int32)
        elif settings.correlatorType == 2:
            # GPU shares an int8 QMBOC code table across all channels.
            B1CCodeTable = np.asarray(B1CCodeTable, dtype=np.int8)

        #%% Initialize waitbar and variables for data reading =============
        # Number of coherent B1C tracking intervals to process.
        numToProcess = trkResults[0].I_P.size
        ...
        if settings.correlatorType == 0:
            ...
            # Compiled SIMD/GPU QMBOC correlators.
            barUprate = 100
            outputType = np.int16
        ...
        # samplesPerCode property and the 100 B1C primary codes per second.
        ...
        for loopCnt in range(numToProcess):
            #%% GUI update -----------------------------------------------
            # Update the GUI periodically without repainting every B1C
            # coherent integration interval.
            if ((loopCnt + 1) % barUprate == 0 or loopCnt + 1 == numToProcess):
                completedMs = round((loopCnt + 1) * settings.intTime * 1000)
                totalMs = round(numToProcess * settings.intTime * 1000)
            ...
            # All backends return an 18 x channelCnt matrix containing the
            # three QMBOC branches in Early/Prompt/Late I/Q order.
            if settings.correlatorType == 0:
                # NumPy reference QMBOC correlator.
                correValues = corrEngine(corrSettings, rawSignal, B1CCodeTable,
                    remCarrPhase, carrPhaseStep,remCodePhase12, codePhaseStep12,
                    startIdx, chSampSize, isDataRead)
            elif settings.correlatorType in (1, 2):
                # CPU SIMD (type 1) or GPU CUDA (type 2) QMBOC correlator.
                correValues = corrEngine.corrEngine(corrSettings, rawSignal,
                    B1CCodeTable,remCarrPhase, carrPhaseStep, remCodePhase12, 
                    codePhaseStep12,startIdx, chSampSize, isDataRead)
            ...
            for channelNr in range(channelCnt):
                ...
                # Data-channel Early, Prompt, and Late outputs.
                I_E, Q_E, I_P, Q_P, I_L, Q_L = (correValues[:6, channelNr])
                # Pilot BOC(1,1) outputs.
                (p11_I_E, p11_Q_E, p11_I_P, p11_Q_P,
                 p11_I_L, p11_Q_L) = (correValues[6:12, channelNr])

                if settings.fullBandEn == 1:
                    # Pilot BOC(6,1) outputs.
                    (p61_I_E, p61_Q_E, p61_I_P, p61_Q_P,
                     p61_I_L, p61_Q_L) = (
                        correValues[12:18, channelNr])
                ...
                # to the next B1C coherent integration interval.
                ...
                #%% Find PLL error and update carrier NCO ------------------
                # Data-channel PLL phase-discriminator output [cycles].
                carrError = np.arctan(Q_P / I_P) / (2.0 * np.pi)
                
                if settings.fullBandEn == 0:
                    # Rotate the narrowband pilot phase back by pi/2.
                    p_QI = (p11_I_P + 1j * p11_Q_P) * np.exp(-1j * np.pi / 2)
                    pCarrError = (np.arctan(np.imag(p_QI) 
                                            / np.real(p_QI))/ (2.0 * np.pi))
                elif settings.fullBandEn == 1:
                    pCarrError = np.arctan(p_Q_P / p_I_P)/ (2.0 * np.pi)
                carrError = carrError * carrFactor + pCarrError * (1 - carrFactor)
                ...
                #%% Find DLL error and update code NCO ---------------------
                # Data-channel Early and Late correlation magnitudes.
                E = np.sqrt(I_E ** 2 + Q_E ** 2)
                L = np.sqrt(I_L ** 2 + Q_L ** 2)
                codeError = (E - L) / (E + L) * (1 - earlyLateSpc)
                ...
                codeError = (codeError * codeFactor + pCodeError * 
                             (1 - codeFactor))
                ...
                #%% C/N0 calculation --------------------------------------
                # Periodically estimate data, pilot, and combined B1C C/N0,
                # together with data/pilot PLL lock-detector values.
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    
                    currentCNoValue, pllDetector = (calcCNoPldQmboc(chResults, 
                                                        settings, loopCnt))
                    cnoCnt = (loopCnt + 1)// settings.CNoVSMinterval - 1
                            
                    # Apply the same 0.5-0.5 C/N0 smoothing used by the
                    # channel-serial B1C tracking path.
                    ...
                    chResults.B1C_CNo[cnoCnt] = (currentCNoValue[2] * 0.5
    ...
    def plotTracking(self):
        ...
        for channelNr, chResults in enumerate(self.trackResults):
            ...
            #--- B1C navigation-message symbols ---------------------------
    ...
    def trackingRun(self, acqResults):
        """Run BDS-3 B1C code and carrier tracking.
        ...
        """
        settings = self.settings
        if settings.intTime != 0.01:
            raise ValueError("BDS-3 B1C tracking requires intTime = 0.01 s")
        ...
        try:
            ...
            if settings.trkMode == 0:
                if settings.correlatorType == 0:
                    ...
                    corrEngine = correlator.corrPySerialB1C
                elif settings.correlatorType == 1:
                    # SIMD correlator
                    corrEngine = correlator.CorrSIMDSerialQMBOC()
                elif settings.correlatorType == 2:
                    # GPU correlator
                    corrEngine = correlator.CorrGPUSerialQMBOC()
            ...
            #--- Channel-parallel tracking --------------------------------
                ...
                if settings.correlatorType == 0:
                    # Python reference correlator
                    corrEngine = correlator.corrPyParallelB1C
                elif settings.correlatorType == 1:
                    # SIMD correlator
                    corrEngine = correlator.CorrSIMDParallelQMBOC()
                elif settings.correlatorType == 2:
                    # GPU correlator
                    corrEngine = correlator.CorrGPUParallelQMBOC()
