...
------------------------------------
Initialize and save BDS-3 B1C receiver settings.
...
class Settings(object):
    """Initialize and store BDS-3 B1C receiver settings.
    ...
    def __init__(self):
        ...
        """
        #%% Processing settings ===============================================
        # Number of milliseconds to be processed. Use 36000 ms plus any
        # transients to ensure that navigation subframes are available.
        self.msToProcess         = 45000          #[ms]
        ...
        # Default raw recording used by the post-processing receiver.
        ...
        self.IF                 = 0.e6             # [Hz]
        self.samplingFreq       = 30.e6            # [Hz]
        # Front-end bandwidth used to calculate the wideband-tracking
        # weighting factor. It is required only in wideband tracking mode.
        self.FEBW               = 25.e6            # [Hz]

        #%% B1C code parameters ==============================================
        # Nominal rate of the B1C primary code.
        self.codeFreqBasis      = 1.023e6          # [Hz]
        # Number of chips in one B1C primary-code period.
        ...
        # Enable acquisition with the pilot channel.
        self.pilotACQflag       = True              # True - On; False - Off
        # List of satellites to look for. Some satellites can be excluded to
        # speed up acquisition.
        self.acqSatelliteList   = np.arange(1,64)   # [PRN numbers]
        # One-sided band around IF searched for satellite signals; the complete
        # search band is twice this value.
        ...
        self.correlatorType     = 0       # 0 - Python correlator;
                                          # 1 - SIMD correlator;
                                          # 2 - GPU correlator;
        # Enable/disable full-band tracking using the BOC(6,1) component.
        self.fullBandEn         = 1       # 0 - Narrowband; 1 - Full band
        ...
        if self.correlatorType == 1:
            self.rShiftBits     = 4       # 0 - No shift;
        # Pickle file used when tracking is skipped.
        self.trkPklName             = "./result_Cache/trackResults.pkl"
        # Code tracking loop parameters
        self.dllDampingRatio        = 0.7
        self.dllNoiseBandwidth      = 1.          # [Hz]
        # Half of the E-L spacing should remain below approximately 0.0765
        # chip in full-band tracking mode (see Betz, 2016).
        self.dllCorrelatorSpacing   = 0.06        # [primary-code chip]
        # Carrier tracking loop parameters
        self.pllDampingRatio        = 0.7
        self.pllNoiseBandwidth      = 18.         # [Hz]
        ...
        #%% CNo Settings ======================================================
        # Number of correlation values used to compute each C/N0 point.
        self.CNoVSMinterval = 50
    ...
    def samplesPerCode(self):
        ...
        -------
            samplesPerCode - int
                           Rounded number of samples in one B1C primary-code
                           period.
        """
    ...
    def postProcessing(self):
        """Process the raw BDS-3 B1C signal and plot the receiver results.
        ...
        1. Acquire the BDS-3 B1C signals.
        ...
        if not self.skipAcquisition:
            ...
            print("   Elapsed time for acquisition is %.0f s\n" %(
        ...
        if not self.skipTracking:
            ...
            print("   Elapsed time for tracking is %.0f s\n" %(
                (datetime.datetime.now() - startTime).total_seconds()) )
