...
#%% B1C C/N0, PLL-lock detector, and DLL weighting
def calcCNoPldQmboc(chResults, settings, loopCnt):
    """Calculate B1C C/N0 and PLL lock-detector outputs.

    C/N0 is estimated with the variance-summing method for the data channel,
    the pilot channel, and the combined B1C signal. The PLL lock detector is
    evaluated separately for the data and pilot channels.
    ...
    ----
        ...
        settings   - object
                     Receiver settings.
        loopCnt    - int
                     Zero-based index of the current integration interval.

    Returns
    -------
        CNo         - numpy.ndarray
                      Estimated C/N0 values in dB-Hz. Elements zero, one, and
                      two correspond to the data channel, pilot channel, and
                      combined B1C signal, respectively.
        pllDetector - numpy.ndarray
                      Data- and pilot-channel PLL detector outputs.
    """
    CNo = np.zeros(3)
    pllDetector = np.zeros(2)
    # Integration time for each correlation point.
    T = settings.intTime
    interval = settings.CNoVSMinterval
    firstIndex = loopCnt - interval + 1
    lastIndex = loopCnt + 1

    #%% C/N0 and PLL-detector estimates for the data channel =================
    # Data-channel prompt correlation values.
    I_P = chResults.I_P[firstIndex:lastIndex]
    Q_P = chResults.Q_P[firstIndex:lastIndex]

    #--- C/N0 estimation for the data channel -------------------------------
    # Calculate power.
    Z = I_P ** 2 + Q_P ** 2
    # Calculate the mean and variance of the power. MATLAB var uses N-1.
    Zm = np.mean(Z)
    Zv = np.var(Z, ddof=1)
    # Calculate the average carrier power and the noise variance.
    # MATLAB continues in the complex domain when ``Zm**2-Zv`` is negative.
    Pav = np.sqrt(complex(Zm ** 2 - Zv))
    Nv = 0.5 * (Zm - Pav)
    # C/N0 estimation for the data channel.
    DataCNo = np.abs((1 / T) * Pav / (2 * Nv))
    CNo[0] = 10 * np.log10(DataCNo)

    #--- PLL lock-detector output -------------------------------------------
    # Narrowband power and narrowband-power difference.
    NBP = (
        (np.sum(I_P[I_P > 0]) - np.sum(I_P[I_P < 0])) ** 2
        + np.sum(Q_P) ** 2
    )
    NBD = (
        (np.sum(I_P[I_P > 0]) - np.sum(I_P[I_P < 0])) ** 2
    ...
    #%% C/N0 and PLL-detector estimates for the pilot channel ================
    if settings.fullBandEn == 1:
        # Pilot-channel prompt correlation values.
        I_P = chResults.Pilot_I_P[firstIndex:lastIndex]
        Q_P = chResults.Pilot_Q_P[firstIndex:lastIndex]
    elif settings.fullBandEn == 0:
        # The narrowband pilot prompt is stored in the quadrature-rotated
        # orientation used by Calc_CNo_PLD_QMBOC.m.
        Q_P = chResults.Pilot_I_P[firstIndex:lastIndex]
        I_P = chResults.Pilot_Q_P[firstIndex:lastIndex]

    #--- C/N0 estimation for the pilot channel ------------------------------
    Z = I_P ** 2 + Q_P ** 2
    Zm = np.mean(Z)
    Zv = np.var(Z, ddof=1)
    # MATLAB continues in the complex domain when ``Zm**2-Zv`` is negative.
    Pav = np.sqrt(complex(Zm ** 2 - Zv))
    Nv = 0.5 * (Zm - Pav)
    PilotCNo = np.abs((1 / T) * Pav / (2 * Nv))
    CNo[1] = 10 * np.log10(PilotCNo)

    #--- PLL lock-detector output for the pilot channel ---------------------
    NBP = (
        (np.sum(I_P[I_P > 0]) - np.sum(I_P[I_P < 0])) ** 2
        + np.sum(Q_P) ** 2
    )
    ...
    #%% C/N0 estimation for the combined B1C signal ==========================
    CNo[2] = 10 * np.log10(DataCNo + PilotCNo)
    return CNo, pllDetector


def calcWeighingFactor(settings):
    """Calculate the data-channel weight for the composite B1C DLL error.

    The weighting factor is used to construct the composite code-tracking
    error in full-band tracking mode.

    Args
    ----
        settings    - object
                    Receiver settings containing the code frequency and
                    front-end bandwidth.
    Returns
    -------
        factor      - float
                    Weighting factor of the data channel.
    """
    ...
    #--- Power ratio and RMS bandwidth of the data channel ------------------
    # PSD of the data-channel BOC(1,1) component. The sinc/tangent form is
    # algebraically identical to CalcWeighingFactor.m and retains its
    # removable limits at f = 0 and the BOC spectral nulls.
    def G_BOC1_1f(f):
        """Return the BOC(1,1) data-channel power spectral density.
        ...
        -------
            psd         - float
                        BOC(1,1) power spectral density at ``f``.
        """
        x = f / fc
        return Tc * (np.tan(np.pi * x / 2) * np.sinc(x)) ** 2

    def G_BOC1_1f_2(f):
        """Return the frequency-weighted BOC(1,1) PSD for RMS bandwidth.
        ...
        -------
            weightedPsd - float
                        Product of the BOC(1,1) PSD and ``f**2``.
        """
        return G_BOC1_1f(f) * f ** 2

    # Integrate the frequency-weighted PSD for the data-channel RMS bandwidth.
    Power_BOC1_1_2 = quad(G_BOC1_1f_2, -Br / 2, Br / 2)[0]
    # Integrated power ratio of the data channel within the front-end band.
    Power_BOC1_1 = quad(G_BOC1_1f, -Br / 2, Br / 2)[0]
    # RMS bandwidth of the data channel.
    REM_BW_BOC_1_1 = np.sqrt(Power_BOC1_1_2 / Power_BOC1_1)

    #--- Power ratio and RMS bandwidth of the pilot channel -----------------
    # The pilot QMBOC PSD consists of 29/33 BOC(1,1) and 4/33 BOC(6,1).
    def G_BOC6_1f(f):
        """Return the BOC(6,1) pilot-component power spectral density.
        ...
        -------
            psd         - float
                        BOC(6,1) power spectral density at ``f``.
        """
        x = f / fc
        return Tc * (np.tan(np.pi * x / 12) * np.sinc(x)) ** 2

    def G_pilotf(f):
        """Return the composite QMBOC pilot power spectral density.
        ...
        -------
            psd         - float
                        Weighted BOC(1,1)/BOC(6,1) pilot PSD at ``f``.
        """
        return 29 / 33 * G_BOC1_1f(f) + 4 / 33 * G_BOC6_1f(f)

    def G_pilotf_2(f):
        """Return the frequency-weighted QMBOC PSD for RMS bandwidth.
        ...
        -------
            weightedPsd - float
                        Product of the QMBOC pilot PSD and ``f**2``.
        """
        return G_pilotf(f) * f ** 2

    # Integrate the frequency-weighted PSD for the pilot RMS bandwidth.
    Power_pilot_2 = quad(G_pilotf_2, -Br / 2, Br / 2)[0]
    # Integrated power ratio of the pilot channel within the front-end band.
    Power_pilot = quad(G_pilotf, -Br / 2, Br / 2)[0]
    # RMS bandwidth of the pilot channel.
    REM_BW_pilot = np.sqrt(Power_pilot_2 / Power_pilot)
...
#%% Angle-format and navigation-plot utilities
def skyPlot(hAxis, az, el, prn, line_style="auto"):
    ...
    for i in range(len(prn)):
        ...
        if prn[i] != 0 and np.any(valid):
            ...
            hAxis.text(xx[i, last], yy[i, last], f"  {int(prn[i])}",color="b")
...
def calculatePseudoranges(trackResults, subFrameStart, TOW, currMeasSample,
    ...
    ----
        ...
        subFrameStart - array-like
                      Zero-based first-frame positions counted in 10 ms
                      B1C primary-code periods from the start of tracking.
        TOW           - array-like
                      Time of week of the first subframe, in seconds.
        currMeasSample - int
                       Current measurement sample location.
        localTime     - float or None
                      Local BDT at the measurement time.
    ...
    for channelNr in channelList:
        ...
        # B1C primary-code period; index-subFrameStart is the integer number
        # of 10 ms periods. Keeping codeLength/codeFreqBasis in the formula
        # preserves the MATLAB BDS timing rather than assuming a 1 ms epoch.
...
def topocent(X, dx):
    ...
    dtr = np.pi/180
    phi, lambda_, _h = togeod(
        6378137, 298.257223563, X[0], X[1], X[2])
...
__all__ = [
    ...
    "WaitBar",
    # Tracking-loop and signal-quality utilities.
    "calcLoopCoef", "calcCNoPldQmboc", "calcWeighingFactor",
    # Navigation plotting.
    "skyPlot",
    # Pseudorange computation.
