...
-----------------------------------
GLONASS L3OC acquisition on CPU or GPU.
...
#%% Acquisition results
class AcqEngine():
    """Perform GLONASS L3OC acquisition and store its results.
    ...
    def __init__(self, settings):
        ...
        # Zero-based L3OC-code phases [samples], shape (svNum,); inf means absent.
    ...
    def acqProcessCPU(self):
        ...
        The function searches for GLONASS L3OC signals of all satellites listed
        ...
        # Actual duration represented by one sampled L3OC code period.
        codePeriod = samplesPerCode * ts
        # Number of samples per L3OC code chip
        ...
        # Map the complete 200 ms fine-acquisition interval to L3OC-code chips
        ...
        L3OcdCode2ms = np.zeros(samplesPerCode * 2, dtype=np.float64)
        L3OcpCode2ms = np.zeros(samplesPerCode * 2, dtype=np.float64)
        ...
        for PRN in settings.acqSatelliteList:
            # Generate L3OCd and L3OCp codes and sample them according to
            # the receiver sampling frequency.
            L3OcdCode2ms[:samplesPerCode] = correlator.codeSampling(
                settings, PRN, samplesPerCode, "data")
            L3OcpCode2ms[:samplesPerCode] = correlator.codeSampling(
                settings, PRN, samplesPerCode, "pilot")
            #--- Perform DFT of L3OCd and L3OCp codes ------------------
            L3OcdCodeFreqDom = np.conjugate(fft.fft(L3OcdCode2ms))
            L3OcpCodeFreqDom = np.conjugate(fft.fft(L3OcpCode2ms))
            ...
            for freqBinIndex in range(numberOfFreqBins):
                ...
                for nonCohIndex in range(settings.acqNonCohTime):
                    ...
                    convL3Ocd = IQfreqDom * L3OcdCodeFreqDom
                    convL3Ocp = IQfreqDom * L3OcpCodeFreqDom
                    # Perform inverse DFT and non-coherently combine the data
                    # and pilot correlation magnitudes.
                    results += (np.abs(fft.ifft(convL3Ocd))
                                + np.abs(fft.ifft(convL3Ocp)))
            ...
            if self.peakMetric[PRN-1] > settings.acqThreshold :
                ...
                # Generate and sample the L3OCd and L3OCp codes over the
                # complete 200 ms fine-acquisition interval.
                L3OcdCode = correlator.generateL3OcdCode(PRN)
                L3OcpCode = correlator.generateL3OcpCode(PRN)
                L3OcdCode200ms = L3OcdCode[codeValueIndex]
                L3OcpCode200ms = L3OcpCode[codeValueIndex]
                ...
                for codePeriodIndex in range(200):
                    ...
                    sumPerCodeData[codePeriodIndex] = np.dot(
                        ...
                        L3OcdCode200ms[signalIndex] * localCarr)
                    sumPerCodePilot[codePeriodIndex] = np.dot(
                        sig200ms[codePeriodIndex],
                        L3OcpCode200ms[signalIndex] * localCarr)
    ...
    def acqProcessGPU(self):
        ...
        # Number of samples in one nominal L3OC code period.
        samplesPerCode = settings.samplesPerCode
        ...
        #--- Variables for fine acquisition -------------------------------
        # Map all 200 ms absolute sample positions to L3OC-code chips once. The
        ...
        L3OcdCode2ms = xp.zeros(samplesPerCode * 2, dtype=realDataType)
        L3OcpCode2ms = xp.zeros(samplesPerCode * 2, dtype=realDataType)
        ...
        #%% Coarse acquisition ============================================
        for PRN in settings.acqSatelliteList:
            # Generate one L3OCd and L3OCp period and sample both at the
            # input sample rate.
            L3OcdCode2ms[:samplesPerCode] = xp.asarray(
                correlator.codeSampling(
                    settings, PRN, samplesPerCode, "data"),
                dtype=realDataType)
            L3OcpCode2ms[:samplesPerCode] = xp.asarray(
                correlator.codeSampling(
                    settings, PRN, samplesPerCode, "pilot"),
                dtype=realDataType)
            #--- Perform DFT of L3OCd and L3OCp codes ------------------
            L3OcdCodeFreqDom = xp.conjugate(fftBackend.fft(L3OcdCode2ms))
            L3OcpCodeFreqDom = xp.conjugate(fftBackend.fft(L3OcpCode2ms))
            ...
            for freqBinIndex in range(numberOfFreqBins):
                ...
                for nonCohIndex in range(settings.acqNonCohTime):
                    ...
                    convL3Ocd = IQfreqDom * L3OcdCodeFreqDom
                    convL3Ocp = IQfreqDom * L3OcpCodeFreqDom
                    # Accumulate L3OCd- and L3OCp-correlation magnitudes.
                    results += (xp.abs(fftBackend.ifft(convL3Ocd))
                                + xp.abs(fftBackend.ifft(convL3Ocp)))
            ...
            if self.peakMetric[PRN-1] > settings.acqThreshold :
                ...
                # Keep one unsampled L3OCd and L3OCp period on the GPU.
                L3OcdCode = xp.asarray(
                    correlator.generateL3OcdCode(PRN),
                    dtype=realDataType)
                L3OcpCode = xp.asarray(
                    correlator.generateL3OcpCode(PRN),
                    dtype=realDataType)

                # Generate the complete continuous 200 ms code replica in one
                # operation. Its values are only +/-1, so float32 is exact.
                L3OcdCode200ms = L3OcdCode[codeValueIndex].reshape(
                    (200, samplesPerCode))
                L3OcpCode200ms = L3OcpCode[codeValueIndex].reshape(
                    (200, samplesPerCode))
                ...
                for codePeriodIndex in range(200):
                    ...
                    localDataCarr = (L3OcdCode200ms[codePeriodIndex]
                                     * localCarr1ms)
                    localPilotCarr = (L3OcpCode200ms[codePeriodIndex]
                                      * localCarr1ms)
