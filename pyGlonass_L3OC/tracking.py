...
--------------------------------
GLONASS L3OC data- and pilot-channel code and carrier tracking.
...
class Channel:
    ...
    def __init__(self, settings):
        ...
        self.Pilot_I_P = np.zeros(msToProcess)
        self.Pilot_Q_P = np.zeros(msToProcess)
        # Loop discriminators
        ...
        # Periodic data, pilot and combined L3OC C/N0 estimates and PLL lock
        # detector outputs.
        cnoCount = msToProcess // settings.CNoVSMinterval
        ...
        self.L3OC_CNo = np.zeros(cnoCount)

#%% Tracking engine
class TrackingEngine():
    """Perform GLONASS L3OC code and carrier tracking.
    ...
    def trkChannelsSerial(self, acqResults, corrEngine):
        ...
        """
        from correlator import generateL3OcdCode, generateL3OcpCode
        ...
        # Summation interval for the code loop.
        ...
        #--- PLL variables ------------------------------------------------
        # Calculate third-order carrier-loop filter coefficients.
        ...
        for channelNr in range(TrackedNr):
            ...
            # Generate guarded L3OC data and pilot primary-code branches.
            L3OcdCode = generateL3OcdCode(chResults.PRN)
            L3OcdCode = np.concatenate(([L3OcdCode[-1]], L3OcdCode,
                                       [L3OcdCode[0]]))
            L3OcpCode = generateL3OcpCode(chResults.PRN)
            L3OcpCode = np.concatenate(([L3OcpCode[-1]], L3OcpCode,
                                       [L3OcpCode[0]]))
            L3OCCodeTable = np.concatenate((L3OcdCode, L3OcpCode))

            if settings.correlatorType == 1:
                # The SIMD DLL consumes a guarded int32 code table.
                L3OCCodeTable = np.asarray(L3OCCodeTable, dtype=np.int32)
            elif settings.correlatorType == 2:
                # The CUDA DLL consumes a guarded int8 code table.
                L3OCCodeTable = np.asarray(L3OCCodeTable, dtype=np.int8)
            ...
            for loopCnt in range(codePeriods):
                ...
                if settings.correlatorType == 0: # NumPy reference correlator
                    correValues = corrEngine(
                        settings, rawSignal, L3OCCodeTable, remCarrPhase,
                ...
                if settings.pilotTRKflag == 1:
                    # The L3OC pilot carrier is pi/2 ahead of the data carrier;
                ...
                # Implement the third-order carrier-loop filter and generate
                # the NCO command, as in trkChannelsSerial.m.
                ...
                if (loopCnt+1)% settings.CNoVSMinterval == 0:
                    ...
                    chResults.PilotCNo[cnoCnt] = (0.5 * CNoValue[1]
                                                  + 0.5 * tempCNoValue[1])
                    chResults.L3OC_CNo[cnoCnt] = (0.5 * CNoValue[2]
                                                 + 0.5 * tempCNoValue[2])
    ...
    def trkChannelsParallel(self, acqResults, corrEngine):
        ...
        # Absolute zero-based sample of the next L3OC-code start in the record.
        ...
        codeLen = settings.codeLength + 2
        L3OCCodeTable = np.empty((channelCnt, 2 * codeLen),
                                dtype=np.float64)
        ...
        for channelNr in range(channelCnt):
            ...
            L3OcdCode = generateL3OcdCode(chResults.PRN)
            L3OcpCode = generateL3OcpCode(chResults.PRN)
            L3OCCodeTable[channelNr] = np.concatenate((
                [L3OcdCode[-1]], L3OcdCode, [L3OcdCode[0]],
                [L3OcpCode[-1]], L3OcpCode, [L3OcpCode[0]]))
        ...
        #%% Initialize tracking variables =================================
        # Number of code periods to process; one GLONASS L3OC period is 1 ms.
        ...
        tau1code, tau2code = calcLoopCoef(
            settings.dllNoiseBandwidth, settings.dllDampingRatio, 1.0)
        #--- PLL variables ------------------------------------------------
        # Carrier-loop coherent integration interval [s].
        ...
        # Number of logical samples per L3OC-code period and per second.
        ...
        for loopCnt in range(codePeriods):
            ...
            if settings.correlatorType == 0:
                ...
                correValues = corrEngine(settings, rawSignal,
                      L3OCCodeTable, remCarrPhase, carrPhaseStep, remCodePhase,
    ...
    def plotTracking(self):
        ...
        for channelNr, chResults in enumerate(self.trackResults):
            ...
            #--- Periodic data, pilot and combined L3OC C/N0 estimates -----
            ...
            CNoAxes.legend(('Data', 'Pilot', 'Combined L3OC'))
            CNoAxes.set(
