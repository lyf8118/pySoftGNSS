...
--------------------------------------
Galileo E5b I/NAV-message decoding and position calculation.
...
class Ephemeris:
    ...
    def NAVdecoding(cls, I_P_InputBits):
        ...
        verified by their expected page spacing. The E5b-I secondary code is
        removed before the I/NAV words are channel decoded.
        ...
        index = np.flatnonzero(np.abs(correlation) > 39.99)
        indexSet = set(index.tolist())

        # Analyze each detected preamble-like pattern.
        for startIndex in index:
            # Verify the expected spacing and available subframe length.
            if (startIndex+1000 not in indexSet
                    or startIndex+30000 > bits.size):
                continue
            # Remove the four-symbol E5b-I secondary code to recover I/NAV.
            inputBits = bits[startIndex:startIndex+30000]
            navBits = (np.sum(
                inputBits.reshape(7500, 4)*secondaryCode, axis=1) < 0
                ).astype(np.uint8)
            syncBits = np.array(
    ...
    def satpos(self, transmitTime):
        ...
        BGD_FIELD = "BGD_E1E5b"
        groupDelay = getattr(self, BGD_FIELD)*(1575.42e6/1207.14e6)**2
        # Calculate clock correction including the E1-E5b group delay.
...
class NavigationEngine:
    ...
    def navigationRun(self, trackResults):
        ...
        # A complete Galileo E5b I/NAV ephemeris set requires at least the
