...
class AcqEngine():
    ...
    def __init__(self, settings):
        ...
        # Zero-based E5b-code phases [samples], shape ``(svNum,)``; inf means
        # unavailable.
    ...
    def acqProcessCPU(self):
        ...
        for PRN in settings.acqSatelliteList:
            ...
            for freqBinIndex in range(numberOfFreqBins):
                ...
                # Non-coherently combine successive two-code input blocks.
                for nonCohIndex in range(settings.acqNonCohTime):
                    signalStart = nonCohIndex * samplesPerCode
                    signal = longSignal[signalStart:signalStart + 2 * samplesPerCode]
            ...
            if self.peakMetric[PRN - 1] > settings.acqThreshold:
                ...
                dataCode = correlator.generateDataCode(PRN, 1)
                pilotCode = correlator.generatePilotCode(PRN, 1)
                dataCodeFine = dataCode[codeValueIndex]
                ...
                basebandData = sigFine * dataCodeFine * localCarrFine
                basebandPilot = sigFine * pilotCodeFine * localCarrFine
                # Galileo fine acquisition forms 200 one-millisecond sums.
                sumData = basebandData.reshape((200, -1)).sum(axis=1)
                sumPilot = basebandPilot.reshape((200, -1)).sum(axis=1)
                spectrum = np.abs(fft.fft(sumData**2)) + np.abs(fft.fft(sumPilot**2))
    ...
    def acqProcessGPU(self):
        ...
        for PRN in settings.acqSatelliteList:
            ...
            if self.peakMetric[PRN - 1] > settings.acqThreshold:
                self.acqFlag[PRN - 1] = True
                print(f"{PRN:02d} ", end="", flush=True)
                dataCode = xp.asarray(correlator.generateDataCode(PRN, 1),
                                      dtype=realDataType)
                ...
                signalCarrier = sigFine * localCarr1ms
                dataCodeFine = dataCode[codeValueIndex].reshape(
                    (200, samplesPer1ms))
                pilotCodeFine = pilotCode[codeValueIndex].reshape(
                    (200, samplesPer1ms))
                sumData = xp.sum(
                    signalCarrier * dataCodeFine, axis=1) * initialCarr
                sumPilot = xp.sum(
                    signalCarrier * pilotCodeFine, axis=1) * initialCarr
                spectrum = (xp.abs(fftBackend.fft(sumData**2))
                            + xp.abs(fftBackend.fft(sumPilot**2)))
