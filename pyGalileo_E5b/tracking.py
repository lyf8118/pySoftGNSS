...
--------------------------------
Galileo E5b code and carrier tracking.
...
class Channel:
    ...
    The results contain data- and pilot-channel prompt outputs, absolute E5b
    ...
    def __init__(self, settings):
        ...
        self.lockFlag = False
        # Absolute input-sample index at the start of each E5b code epoch.
        ...
        self.E5b_CNo = np.zeros(cnoCount)

#%% Tracking engine
class TrackingEngine():
    """Perform Galileo E5b code and carrier tracking.
    ...
    def trkChannelsSerial(self, acqResults, corrEngine):
        """Perform Galileo E5b code and carrier tracking for selected channels.

        The channels are processed using channel-serial tracking mode. Each
        correlator call processes the E5bI data and E5bQ pilot branches on the
        ...
        # Determine how many acquired E5b channels will be tracked.
        ...
        # Number of preallocated coherent E5b primary-code integrations.
        ...
        for channelNr in range(TrackedNr):
            # Mutable result container for the current E5b channel.
            ...
            # Get E5bI and E5bQ codes sampled once per primary-code chip and
            # add one guard chip at each end for Early/Late indexing.
            E5bICode = generateDataCode(chResults.PRN)
            E5bICode = np.concatenate(
                ([E5bICode[-1]], E5bICode, [E5bICode[0]]))
            E5bQCode = generatePilotCode(chResults.PRN)
            E5bQCode = np.concatenate(
                ([E5bQCode[-1]], E5bQCode, [E5bQCode[0]]))
            E5bCodeTable = np.concatenate((E5bICode, E5bQCode))
            if settings.correlatorType == 1:
                E5bCodeTable = np.asarray(E5bCodeTable, dtype=np.int32)
            elif settings.correlatorType == 2:
                E5bCodeTable = np.asarray(E5bCodeTable, dtype=np.int8)
            ...
            for loopCnt in range(numToProcess):
                ...
                # it for every coherent E5b integration interval.
                ...
                # Record the absolute sample index of this E5b code epoch.
                ...
                if settings.correlatorType == 0:
                    ...
                    correValues = corrEngine(
                        corrSettings, rawSignal, E5bCodeTable, remCarrPhase,
                ...
                # E5bQ is pi/2 ahead of E5bI; rotate it back before combining.
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # A 0.5-0.5 filter smooths data, pilot, and E5b C/N0.
                    ...
                    chResults.E5b_CNo[cnoCnt] = (CNoValue[2] * 0.5
                                                 + tempCNoValue[2] * 0.5)
    ...
    def trkChannelsParallel(self, acqResults, corrEngine):
        """Perform Galileo E5b tracking for all active channels.
        ...
        # Number of active E5b channels to be tracked.
        ...
        # Primary E5b code length in chips.
        codeLength = settings.codeLength
        corrSettings = settings

        #%% Generate E5b code tables for acquired signals ================
        # Each row contains guarded E5bI and E5bQ primary-code branches.
        branchLength = codeLength + 2
        E5bCodeTable = np.empty(
        ...
        for channelNr in range(channelCnt):
            ...
            E5bQCode = np.concatenate(
                ([E5bQCode[-1]], E5bQCode, [E5bQCode[0]]))
            E5bCodeTable[channelNr] = np.concatenate((E5bICode, E5bQCode))
        ...
        # Number of coherent E5b tracking intervals to process.
        numToProcess = trkResults[0].I_P.size
        ...
        bytesPerSample = dataAdaptCoeff * sampleType.itemsize
        # Number of logical samples per E5b-code period and per second.
        ...
        for loopCnt in range(numToProcess):
            #%% GUI update -----------------------------------------------
            # Update the GUI periodically without repainting every E5b
            # coherent integration interval.
            if ((loopCnt + 1) % barUprate == 0 or loopCnt + 1 == numToProcess):
                completedMs = round((loopCnt + 1) * settings.intTime * 1000)
                totalMs = round(numToProcess * settings.intTime * 1000)
            ...
            # All backends return a 12 x channelCnt matrix containing E5bI
            # and E5bQ Early/Prompt/Late I/Q correlations.
            if settings.correlatorType == 0:
                # NumPy reference QPSK correlator.
                correValues = corrEngine(
                    corrSettings, rawSignal, E5bCodeTable, remCarrPhase,
            ...
            for channelNr in range(channelCnt):
                ...
                # E5bI and E5bQ Early, Prompt, and Late outputs.
                ...
                # to the next E5b coherent integration interval.
                ...
                #%% C/N0 calculation --------------------------------------
                # Periodically estimate data, pilot, and combined E5b C/N0,
                ...
                if ((loopCnt + 1) % settings.CNoVSMinterval == 0):
                    ...
                    # channel-serial E5b tracking path.
                    ...
                    chResults.E5b_CNo[cnoCnt] = (currentCNoValue[2] * 0.5
    ...
    def plotTracking(self):
        ...
        for channelNr, chResults in enumerate(self.trackResults):
            ...
            #--- E5b navigation-message symbols --------------------------
    ...
    def trackingRun(self, acqResults):
        """Run Galileo E5b code and carrier tracking.
