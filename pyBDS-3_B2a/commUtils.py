...
def calcCNoPld(trackResults, settings, loopCnt):
    """Calculate B2a data, pilot and combined C/N0 and PLL lock metrics.

    Args
    ----
        trackResults - object
                     Correlation values for one tracking channel.
        settings     - object
                     Receiver settings.
        loopCnt      - int
                     Zero-based iteration index for this C/N0 calculation.
    Returns
    -------
        CNo          - numpy.ndarray
                     Data, pilot and combined B2a C/N0 values in dB-Hz.
        pllDetector  - numpy.ndarray
                     Data- and pilot-channel PLL lock-detector outputs.
    """
    interval = settings.CNoVSMinterval
    first = loopCnt - interval + 1
    CNo = np.zeros(3)
    pllDetector = np.zeros(2)

    #--- C/N0 and PLL detector estimations for the data channel ----------
    I_P = trackResults.I_P[first:loopCnt + 1]
    Q_P = trackResults.Q_P[first:loopCnt + 1]
    Z = I_P**2 + Q_P**2
    Zm = np.mean(Z)
    Zv = np.var(Z, ddof=1)
    Pav = np.emath.sqrt(Zm**2 - Zv)
    Nv = 0.5 * (Zm - Pav)
    dataCNo = np.abs((1 / settings.intTime) * Pav / (2 * Nv))
    CNo[0] = 10 * np.log10(dataCNo)

    absISum = np.sum(I_P[I_P > 0]) - np.sum(I_P[I_P < 0])
    NBP = absISum**2 + np.sum(Q_P)**2
    NBD = absISum**2 - np.sum(Q_P)**2
    pllDetector[0] = NBD / NBP

    #--- C/N0 and PLL detector estimations for the pilot channel ---------
    pilotCNo = 0.0
    if settings.pilotTRKflag == 1:
        Q_P = trackResults.Pilot_I_P[first:loopCnt + 1]
        I_P = trackResults.Pilot_Q_P[first:loopCnt + 1]
        Z = I_P**2 + Q_P**2
        Zm = np.mean(Z)
        Zv = np.var(Z, ddof=1)
        Pav = np.emath.sqrt(Zm**2 - Zv)
        Nv = 0.5 * (Zm - Pav)
        pilotCNo = np.abs((1 / settings.intTime) * Pav / (2 * Nv))
    ...
    # C/N0 estimate for the combined B2a signal.
    CNo[2] = 10 * np.log10(dataCNo + pilotCNo)
    return np.real(CNo), np.real(pllDetector)
...
def calculatePseudoranges(trackResults, subFrameStart, TOW, currMeasSample,
    ...
    ----
        ...
        subFrameStart - array-like
                      Zero-based first-preamble positions counted in 1 ms
                      B2a-code periods from the start of tracking.
        TOW           - array-like
